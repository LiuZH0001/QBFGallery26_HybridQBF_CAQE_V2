...
struct Constraint
{
  ...
  /* Multi-purpose mark used for QBCE: for elimination of universal
     ...
     用于 QBCE（量化布尔公式消除，Quantified Boolean Constraint Elimination）的多用途标记：
     既用于在学习过程中从赋值中消除全称量词字面量，也用于重新安排 QBCE 中待再次检查的子句。*/
  unsigned int qbcp_qbce_multi_purpose_mark:1;
  unsigned int num_lits:(sizeof (unsigned int) * 8 - 4);
  unsigned int glue: (sizeof(unsigned int) * 8 - 4);
  double avg_activity;
  double avg_scope;
  int activity;//学习子句或cube的活跃度
