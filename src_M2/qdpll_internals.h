...
typedef struct SAT_lit1 SAT_lit1;
...
typedef struct SAT_clause1 SAT_clause1;

struct QDPLL
{
    //学习子句和cube删除
    //int ls_call_numbers;//学习子句调用频率
    //double time_of_hash;//hash占多少时间
    //double ls_call_time;//ls占多少时间
    //double update_time;
    int use_clause_cube_delete;
    int* undeletable;
    int num_undeletable;
    int max_num_undeletable;
    _Node* undeletable_array;
    double sort_time;
    int delete_mode;
    int use_clause_cube_delete_after_restart;
    int delete_num;
    int clause_move;
    int just_start;
    ...
    //嵌入LS
    int use_ls_to_find_true_after_forall_assign;
    int num_res_noassign_forall;
    double time_of_call_ls = 0;
    int* vis;
    int call_ls_num;
    int ls_find_true, ls_find_false;
    long long ls_r;
    int no_qcdcl_guide_ls;
    int only_phase_saving;

    int use_individual_heuristic;
    //为1使用任意文字多满足子句
    int branch_in_forall;
    int first_branch_in_forall;
    int** backtrack_state_SAT, ** backtrack_state_UNSAT;//要分取值为-1还是为1
    int* _polarity;
    int* _polarity_LS;
    int current_state;
    int *back_need_collect;
    int* tail_saving;//只记录决策变量
    int num_tail_saving;//只记录决策变量个数
    int* is_decision_variable;//标记是否为决策文字，用于维护num_tail_saving
    int skip_variable;//上一次被传播的变量，应该要被跳过
    int* belong;//标记变量在这个子句里面

    int all_call_ls_num;

    int use_variable_activity;
    int variable_activity_decay_ratio;
    int* variable_activity;
    int* tend_ass_true, * tend_ass_false;
    int num_backtrack_zero;

    int glue1, glue2, glue3, glue4, glue5, glue6;
    int glueg[101];
    //double avg_lclause = 0; double avg_lcube = 0;
    //double len_lclause = 0; double len_lcube = 0;//学习子句和cube长度
    //double lbd_lclause = 0; double lbd_lcube = 0;//学习子句和cube的lbd
    //double num_lclause = 0; double num_lcube = 0;//学习子句和cube个数
    //double num_discarded_clause_cube;
    //int record_decide_level_conf;
    //int record_decide_level_solu;
    //int record_ass_num_conf;
    //int record_ass_num_solu;
    //int record_conf_back;
    //int is_time_exit;
    //int num_aaa;
    int* glue_decide_level;
    int num_false, num_true, num_undef;
    unsigned int seed;
    char* in_filename;
    double start, end, run_time;//时间
  ...
  struct CCaDiCaL* trivial_falsity_solver;
  struct CCaDiCaL* trivial_truth_solver;
  ...
  /* Data for fast check of stop-resolution. 用于快速检查停止解析（stop-resolution）的数据。*/
