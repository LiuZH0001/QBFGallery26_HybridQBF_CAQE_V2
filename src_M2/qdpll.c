...
//#include "./kissat-master/src/kissat.h"
#include "./cadical-master/src/ccadical.h"
...
int walksat_max(int a, int b) {
    return a > b ? a : b;
}

int walksat_min(int a, int b) {
    return a < b ? a : b;
}
  ...
  for (p = qdpll->pcnf.vars, e = p + qdpll->pcnf.size_vars; p < e; p++)
    {
      /* All variables which are candidates must be either (already)
         ...
         所有作为候选的变量，要么（已）被赋值，要么必须出现在优先队列中。*/
      if (p->id)
  ...
  if ((old_size = qdpll->size_var_pqueue) < size)
    {
      ...
      qdpll->var_pqueue = (decltype(qdpll->var_pqueue)) qdpll_realloc (mm, qdpll->var_pqueue,
...
var_pqueue_compare (QDPLL * qdpll, unsigned int pos_a, unsigned int pos_b)
{
  ...
  if (qdpll->use_variable_activity)
  {
      ...
      {
          int var_a_scope = var_a->scope->nesting;
          int var_b_scope = var_b->scope->nesting;
          if (var_a_scope < var_b_scope)
              return -1;
          else if (var_a_scope == var_b_scope)
          {
              int var_a_ls_activity = qdpll->variable_activity[var_a->id];
              int var_b_ls_activity = qdpll->variable_activity[var_b->id];
              if (var_a_ls_activity > var_b_ls_activity)
                  return -1;
              else if (var_a_ls_activity == var_b_ls_activity)
              {
...
{
  QDPLLMemMan *mm = qdpll->mm;
  Constraint *result = (Constraint*) qdpll_malloc (mm,
                                     sizeof (Constraint) +
                        ...
                        QDPLLVarMode mode)
{
  ...
  qdpll->_polarity[var->id] = (int)assignment;
  qdpll->_polarity_LS[var->id] = (int)assignment;
  //if (assignment == QDPLL_ASSIGNMENT_FALSE)  qdpll->_polarity[var->id] = 0;
  //else if (assignment == QDPLL_ASSIGNMENT_TRUE)  qdpll->_polarity[var->id] = 1;
  var->mode = mode;
...
static void set_learnt_constraint_glue(QDPLL* qdpll, Constraint* learnt_constraint)
{
    //learnt_constraint->glue = 1;
    return;
    Var* vars = qdpll->pcnf.vars, * var;
    LitID* p, * e, lit;
    int level, num = 0, num_0 = 0;
    //num_0代表有多少未赋值，当使用TT或TF或nenofex会使得下面的var可能在qcdcl中没有被赋值
    for (e = learnt_constraint->lits, p = e + learnt_constraint->num_lits - 1; e <= p; p--)
    {
        var = LIT2VARPTR(vars, *p);
        //printf("var->id = %d", var->id);
        level = var->decision_level + 1;
        qdpll->glue_decide_level[level]++;
        if(qdpll->glue_decide_level[level]==1 && level > 0)  num++;
    }

    learnt_constraint->glue = num;// +qdpll->glue_decide_level[0];

    for (e = learnt_constraint->lits, p = e + learnt_constraint->num_lits - 1; e <= p; p--)
    {
        var = LIT2VARPTR(vars, *p);
        level = var->decision_level + 1;
        qdpll->glue_decide_level[level] = 0;
    }
    /*
    if (learnt_constraint->is_cube == 1)
    {
        qdpll->num_lcube += 1, qdpll->len_lcube += learnt_constraint->num_lits;
        qdpll->lbd_lcube += learnt_constraint->glue;
    }
    else {
        qdpll->num_lclause += 1, qdpll->len_lclause += learnt_constraint->num_lits;
        qdpll->lbd_lclause += learnt_constraint->glue;
    }*/
}

static void set_learnt_constraint_activity(QDPLL* qdpll, Constraint* learnt_constraint)
{
    //return;
    Var* vars = qdpll->pcnf.vars, * var;
    LitID* p, * e, lit;
    int level, num = learnt_constraint->num_lits;
    double sum = 0;
    //num_0代表有多少未赋值，当使用TT或TF或nenofex会使得下面的var可能在qcdcl中没有被赋值
    for (e = learnt_constraint->lits, p = e + learnt_constraint->num_lits - 1; e <= p; p--)
    {
        var = LIT2VARPTR(vars, *p);
        sum += var->priority / num;
        //printf("var->priority=%.6lf    num=%d\n", var->priority, num);
    }
    learnt_constraint->avg_activity = sum;
    //printf("sum = %.6lf\n", sum);
    //exit(0);
}

static void set_learnt_constraint_scope(QDPLL* qdpll, Constraint* learnt_constraint)
{
    //return;
    Var* vars = qdpll->pcnf.vars, * var;
    LitID* p, * e, lit;
    ...
    {
        var = LIT2VARPTR(vars, *p);
        sum += var->scope->nesting;
        num++;
        //printf("var->priority=%.6lf    num=%d\n", var->priority, num);
    }
    learnt_constraint->avg_scope = sum / num;
    //printf("sum = %.6lf\n", sum);
    //exit(0);
}
...
learnt_constraint_mtf (QDPLL * qdpll, Constraint * c)
{
  ...
  if (!c->glue)
  {
      set_learnt_constraint_glue(qdpll, c);
  }*/
  if (c->is_cube)
    {
      c->activity++;
                       ...
                       const QDPLLQuantifierType type)
{
  ...
  if (var_type == type)
  ...
  else
    {
      /* Collect literals of other type which potentially violate
         ...
         最后收集可能违反停止条件（stop-crit）的其他类型的字面量。*/
      if (!QDPLL_VAR_ASSIGNED (var)
  ...
  /* Note: when using simple dep-man, then need not collect data for
     ...
     注意：使用简单依赖管理器（simple dep-man）时，无需收集用于类型归约（type-reduce）的数据。
     实际上，若我们收集了这类数据，就还必须正确地为变量等取消标记，而目前这一步骤尚未实现。*/
...
/* Collect cover-lits already in-order by traversing scopes. This
   ...
   通过遍历作用域（scopes）收集已按顺序排列的覆盖字面量（cover-lits），
   这样可避免对立方体字面量（cube-literals）进行可能成本高昂的排序操作。*/
static void
                                      ...
                                      LitIDStack * lit_stack)
{
  /* Data for stop-crit and type-reduce must be reset and will be
     initialized here.
     停止条件（stop-crit）和类型归约（type-reduce）所需的数据必须重置，并将在此处完成初始化。*/
  ...
  /* First import internal variables collected at separate stack. Separate
     ...
     首先导入在独立栈（separate stack）中收集的内部变量（internal variables）。这种独立收集方式是必要的，
     目的是让内部字面量（internal literals）出现在约束字面量列表（constraint literal lists）的左端。*/
  ...
  然后收集添加到默认作用域（default scope）的变量。*/
  cover_by_clauses_collect_lits_sorted_aux (qdpll, mm,
	  ...
	  !qdpll->state.sat_branch_detected)
        {
          /* No literal is marked in innermost scope, since that
             ...
             最内层作用域（innermost scope）中没有标记任何字面量（literal），因为这些字面量会被类型归约（type-reduce）操作立即移除。*/
  ...
  /* Re-collect all marked literals by traversing all scopes from
     ...
     通过从外层到最内层遍历所有作用域，重新收集所有已标记的字面量。
     这样，已标记的字面量就能按作用域顺序被收集，从而可避免显式排序。*/
  Scope *s;
  for (s = qdpll->pcnf.user_scopes.first; s; s = s->link.next)
    cover_by_clauses_collect_lits_sorted_aux (qdpll, mm,
              ...
              if (QDPLL_VAR_FORALL (var))
                  ...
                  if (var->mode != QDPLL_VARMODE_PURE && !min_sat_univ_var)
                    {
                      /* Found a satisfying universal literal. If there are
                         ...
                         找到了一个可满足的全称量词字面量（satisfying universal literal）。若存在多个此类字面量，
                         则仅存储最左侧的那个（需注意，我们会从左至右遍历字面量），
                         这与传统 QCDCL 立方体学习（traditional QCDCL cube learning）中的启发式策略类似。*/
              ...
              else
                {
                  /* NOTE: found a satisfying existential literal which has
                     ...
                     注意：发现了一个尚未被收集的可满足存在量词字面量（satisfying existential literal）。
                     但这种情况理论上不应发生，因为我们已提前收集了所有存在量词字面量。*/
                  assert (QDPLL_VAR_EXISTS (var));
                  assert (LEARN_VAR_MARKED (var));
                                       ...
                                       ConstraintPtrStack *marked_clauses_stack)
{
  ...
  /* Check original occurrences of universal variables, collect universals
     ...
     检查全称变量的原始出现情况，收集覆盖这些出现所必需的全称变量。*/
  while (!QDPLL_EMPTY_STACK (*check_clauses_stack))
    {
#if COMPUTE_STATS
      qdpll->stats.elim_univ_vars_clauses_seen++;
  ...
  /* Collect universals reachable from blocked clauses. First collect blocked
     ...
     收集可从阻塞子句（blocked clauses）访问到的全称量词变量。首先收集作为遍历起点的阻塞子句。*/
  ConstraintPtrStack *sp, *se;
  for (sp = qdpll->qbcp_qbce_blocked_clauses.start, 
         se = qdpll->qbcp_qbce_blocked_clauses.top; sp < se; sp++)
    {
  ...
  从已收集的阻塞根节点子句（blocked root clauses）开始遍历子句。*/
  while (!QDPLL_EMPTY_STACK (*check_clauses_stack))
    {
#if COMPUTE_STATS
      qdpll->stats.elim_univ_vars_clauses_seen++;
      ...
      if (c->qbcp_qbce_blocked)
      ...
      else
        {
          ...
          /* Collect universals from non-blocked clauses and push new clauses
             ...
             从非阻塞子句（non-blocked clauses）中收集全称量词变量，
             并从存在量词纯字面量（existential pure literals）中推送新子句。*/
          LitID *p, *e;
          for (p = c->lits, e = p + c->num_lits; p < e; p++)
            {
              LitID lit = *p;
                  ...
                  if (QDPLL_VAR_FORALL (var) && !LEARN_VAR_MARKED (var) &&
                  ...
                  else if (QDPLL_VAR_EXISTS (var) && var->mode == QDPLL_VARMODE_PURE)
                    {
                      /* Push other occurrences of pure variable. A universal
                         ...
                         推送纯变量的其他出现情况。一个全称量词变量可能会触发某个存在量词变量变为纯变量，
                         而这种纯赋值反过来可能会产生更多的阻塞子句。因此，我们可能需要收集那个全称量词变量。*/
                      BLitsOccStack *occs = QDPLL_VAR_ASSIGNED_FALSE (var) ? 
...
cover_by_assignment_toggle_dynamic_elim_univs (QDPLL *qdpll)
{
  /* Check whether to turn off elimination of universal variables based on
     ...
     根据（消除全称变量的）成功率来决定是否关闭该功能，前提是此前尚未关闭。*/
...
/* Take the current assignment as initial cube. This approach is intended for
   ...
   将当前赋值作为初始立方（cube）。这种方法旨在与QBCE（量化阻塞子句消除）结合使用，因为某些输入子句可能被阻塞。
   若不采用QBCE技术，则'cover_by_clauses'方法可能会产生更优（？）的立方，因为该方法会优先选择存在量词文字。*/
static int
cover_by_assignment (QDPLL * qdpll, LitIDStack * lit_stack)
{
  ...
  /* First collect ALL assigned existential variables in the initial cube. Then
     ...
     them. 
     首先收集初始立方体（initial cube）中所有已赋值的存在量词变量（assigned existential variables）。
     然后遍历已赋值的全称量词变量（assigned universal variables），检查是否可以排除其中的一部分。*/
  VarPtrStack univ_vars_stack;
  ...
  /* Must collect EVERY enqueued assignment, even those not yet propagated,
     ...
     assignment. 
     必须收集所有已入队的赋值（enqueued assignment），即使是那些尚未传播的赋值也不例外。
     因为我们使用空公式监视（empty formula watching）来检测当前赋值下的空合取范式（empty CNF）。*/
  QDPLLMemMan *mm = qdpll->mm;
  VarID *p, *e;
  for (p = qdpll->assigned_vars, e = qdpll->assigned_vars_top; p < e; p++)
    {
                  ...
                  if (lit_var->scope == last_scope)
                    {
                      if (qdpll->options.trace && !lit_var->mark_qrp)
                        {
                          lit_var->mark_qrp = 1;        /* prevent duplicates   防止重复 */
      ...
      if (max_e_true_var)
        {
          ...
          /* Clause is not covered by already collected lit (otherwise we
             ...
             该子句未被已收集的文字量覆盖（否则我们本该跳至下方的 “SKIP” 标签处）。
             需收集一个新变量，设置标记：若当前子句恰好由该变量满足，则避免尝试从覆盖范围中移除这个已收集的变量。*/
	  ...
	  if (res)
      ...
      else
        {
          /* NOTE: this branch is taken if we successfully applied
             ...
             enabled. 
             注意：如果动态 Nenofex 测试或平凡真值判定成功应用，或者启用了动态 QBCE，则执行此分支。*/
          ...
          /* NOTE: returned value 'res' is relevant only for tracing in
             ...
             printed. 
             返回值 'res' 仅在传统 QCDCL 中调用上述 'cover_by_clauses' 函数进行追踪时具有意义。
             该返回值用于标示内部存在量词文字的动态消除情况，追踪文件中会为此生成单独的记录条目。*/
          assert (!res);
        }
...
qpup_select_next_node (QDPLL *qdpll)
{
  ...
  result = (Var*)pqueue_remove_min (qdpll->qpup_nodes);
  ...
  if (!qdpll->qpup_uip)
    {
      ...
      if (qdpll->qpup_cnt_at_max_dec_level == 1)
        qdpll->qpup_var_at_max_dec_level = (Var*)pqueue_access_min (qdpll->qpup_nodes);
    }
                        ...
                        unsigned int asserting_level, const QDPLLQuantifierType type)
{
  ...
  //set_learnt_constraint_glue(qdpll, qpup_constraint);

  /* Prepend to list of constraints. */
  if (!qpup_constraint->is_cube)
  {
      qpup_constraint->activity++;
...
generate_and_add_reason (QDPLL * qdpll, const QDPLLQuantifierType type)
{
  ...
  {
      Var* var = LIT2VARPTR(vars, *p);
      if (var->assignment == QDPLL_ASSIGNMENT_UNDEF)//有这种情况，因为TT、TF或者nenofex
      {
          printf("type=%d     qdpll->state.unsat_branch_clause=%d   qdpll->result_constraint=%d\n", type, qdpll->state.unsat_branch_clause, qdpll->result_constraint);
          printf("%d->assignment == QDPLL_ASSIGNMENT_UNDEF!!!    size = %d\n", var->id, lit_stack->top - lit_stack->start);
          exit(0);
      }
  }*/
  ...
  if (success)
    {
      ...
      //set_learnt_constraint_glue(qdpll, learnt_constraint);

      if (type == QDPLL_QTYPE_EXISTS)
        {
          learnt_constraint->activity++;
...
backtrack_undo_assignment (QDPLL * qdpll, Var * var, const int notify_active)
{
  ...
  //if ((var->mode == QDPLL_VARMODE_LBRANCH || var->mode == QDPLL_VARMODE_RBRANCH) && qdpll->current_state != 0 && qdpll->back_need_collect[var->id] == 1)
  //if (qdpll->current_state != 0 && qdpll->back_need_collect[var->id] == 1)
  if (qdpll->current_state != 0)
  {
      //var是决策变量，且当前状态是SAT或UNSAT，且需要记录
      int val = (var->assignment + 1) / 2;//建立-1 -> 0, 1 -> 1 的映射
      int vid = var->id;
      if (qdpll->current_state == 1)
      {
          //现在是SAT
          qdpll->backtrack_state_SAT[val][vid]++;
          //printf("  SAT: %10d  %10d  %10d  %10d\n", vid, val, qdpll->backtrack_state_SAT[val][vid], qdpll->backtrack_state_UNSAT[val][vid]);
      }
      else {
          //现在是UNSAT
          qdpll->backtrack_state_UNSAT[val][vid]++;
          //printf("UNSAT: %10d  %10d  %10d  %10d\n", vid, val, qdpll->backtrack_state_SAT[val][vid], qdpll->backtrack_state_UNSAT[val][vid]);
      }
  }
   if (qdpll->is_decision_variable[var->id] == 1)
   {
       qdpll->num_tail_saving--;
       qdpll->is_decision_variable[var->id] = 0;
   }
...
{
    //核心思想：先将所有无依赖、可决策的变量放在候选集candidate中，然后将所有的候选变量放入优先队列中，最后选择优先级最大的变量。
    //目前DepQBF采用的依赖关系是“标准依赖关系”,而不是“三角依赖关系”
  QDPLLDepManGeneric *dm = qdpll->dm;
  ...
  while (QDPLL_VAR_ASSIGNED (decision_var)
         || !dm->is_candidate (dm, decision_var_id));//属于候选集且未被赋值
...
static void mark_neightbor(QDPLL* qdpll, BLitsOccStack* neg_occs, BLitsOccStack* pos_occs)
{
    unsigned int sum = 0;
    BLitsOcc* bp, * be;
    for (bp = neg_occs->start, be = neg_occs->top; bp < be; bp++)
    {
        if (bp->constraint->qbcp_qbce_blocked || is_clause_satisfied(qdpll, bp->constraint))  continue;//被阻塞或者被满足了
        qdpll->belong[bp->constraint->id] = 1;
    }
    for (bp = pos_occs->start, be = pos_occs->top; bp < be; bp++)
    {
        if (bp->constraint->qbcp_qbce_blocked || is_clause_satisfied(qdpll, bp->constraint))  continue;//被阻塞或者被满足了
        qdpll->belong[bp->constraint->id] = 1;
    }
...
static void mark_neightbor_clean(QDPLL* qdpll, BLitsOccStack* neg_occs, BLitsOccStack* pos_occs)
{
    unsigned int sum = 0;
    BLitsOcc* bp, * be;
    for (bp = neg_occs->start, be = neg_occs->top; bp < be; bp++)
    {
        qdpll->belong[bp->constraint->id] = 0;
    }
    for (bp = pos_occs->start, be = pos_occs->top; bp < be; bp++)
    {
        qdpll->belong[bp->constraint->id] = 0;
...
static int neightbor_check_aux(QDPLL* qdpll, BLitsOccStack* neg_occs, BLitsOccStack* pos_occs)
{
    //这里不需要再检查子句是否被阻塞或者被满足了
    //因为，如果x和y真的在（未阻塞 且 未满足）的子句中，是邻居
    //那么那些（被阻塞 或 已满足）的子句，在前面的检查里，不会被x标记
    unsigned int sum = 0;
    BLitsOcc* bp, * be;
    for (bp = neg_occs->start, be = neg_occs->top; bp < be; bp++)
    {
        if (qdpll->belong[bp->constraint->id] == 1)  return 1;
    }
    for (bp = pos_occs->start, be = pos_occs->top; bp < be; bp++)
    {
        if (qdpll->belong[bp->constraint->id] == 1)  return 1;
...
static int neightbor_check_in_walksat(WalkSAT* walksat, int x, int y)
{
    int* p;
    int v;
    //printf("x=%d y=%d\n",x,y);
    if (walksat->var_neighbor_count[x] == 0)  return 0;
    for (p = walksat->var_neighbor[x]; (v = *p) != 0; p++)
    {
        //printf("v=%d  walksat->var_neighbor[x]=%d", v, walksat->var_neighbor[x]);
        if (v == y)  return 1;
    }
    return 0;
}

static int neightbor_check(QDPLL* qdpll, WalkSAT* walksat, int x, int y)
{
    //printf("111\n");
    //现在walksat数据结构中检查x和y是不是邻居，这个检查快一些
    //int flag;
    int flag  = neightbor_check_in_walksat(walksat, x, y);
    //printf("flag = %d    x=%d  y=%d\n",flag,x,y);
    if (flag == 0)  return flag;//不是邻居
    //printf("222\n");
    
    Var* vx = LIT2VARPTR(qdpll->pcnf.vars, x);
    Var* vy = LIT2VARPTR(qdpll->pcnf.vars, y);
    //标记邻居
    //printf("222.5\n");
    mark_neightbor(qdpll, &(vx->neg_occ_clauses), &(vx->pos_occ_clauses));
    //printf("333\n");

    flag = neightbor_check_aux(qdpll, &(vy->neg_occ_clauses), &(vy->pos_occ_clauses));
    //printf("444\n");

    //清空邻居标记
    mark_neightbor_clean(qdpll, &(vx->neg_occ_clauses), &(vx->pos_occ_clauses));
    //printf("555\n");
...
select_decision_assignment (QDPLL * qdpll, Var * decision_var)
{
  if (qdpll->options.dh == QDPLL_DH_SIMPLE)
  ...
  else
    {
      ...
      {
          //下面的branch_in_forall=1，代表使用 <任意多> 策略
          if(qdpll->use_individual_heuristic == 1 && decision_var->scope->type == QDPLL_QTYPE_FORALL && qdpll->branch_in_forall == 1)  
          //下面的branch_in_forall=1，代表使用 <任意多，存在少> 策略
          //if (qdpll->use_individual_heuristic == 1 && qdpll->branch_in_forall == 1)
              return -1 * qtype_dec_heuristic(qdpll, decision_var);
          //这里depqbf原来用的策略是任意文字少满足子句，存在变量少满足子句，即 <任意少>
...
void qdpll_swap(QDPLL* qdpll, int low, int high) //交换两个数的值
{
    if (low == high)
        return;
    double t = qdpll->undeletable_array[low].score;
    int t1 = qdpll->undeletable_array[low].order;
    qdpll->undeletable_array[low].score = qdpll->undeletable_array[high].score;
    qdpll->undeletable_array[low].order = qdpll->undeletable_array[high].order;
    qdpll->undeletable_array[high].score = t;
    qdpll->undeletable_array[high].order = t1;
    if (qdpll->undeletable_array[high].order == 0 || qdpll->undeletable_array[low].order == 0)
    {
        printf("qdpll->undeletable_array[high].order == 0 || qdpll->undeletable_array[low].order == 0\n");
        printf("low=%d  high=%d\n", low, high);
        exit(0);
    }
...
void qdpll_adjust(QDPLL* qdpll, int index, int len)
{
    ...
    if (leftIndex < len)
    {
        if (qdpll->undeletable_array[bigIndex].score < qdpll->undeletable_array[leftIndex].score)  bigIndex = leftIndex;
        else if( my_fabs(qdpll->undeletable_array[bigIndex].score - qdpll->undeletable_array[leftIndex].score) < 0.000001 && 
            qdpll->undeletable_array[bigIndex].order < qdpll->undeletable_array[leftIndex].order)
            bigIndex = leftIndex;
    }

    if (rightIndex < len)
    {
        if(qdpll->undeletable_array[bigIndex].score < qdpll->undeletable_array[rightIndex].score)  bigIndex = rightIndex;
        else if(my_fabs(qdpll->undeletable_array[bigIndex].score - qdpll->undeletable_array[rightIndex].score) < 0.000001 &&
            qdpll->undeletable_array[bigIndex].order < qdpll->undeletable_array[rightIndex].order)
            bigIndex = rightIndex;
    }

    if (bigIndex != index) {
        qdpll_swap(qdpll, index, bigIndex);
        qdpll_adjust(qdpll, bigIndex, len);
    }
}

void qdpll_heapsort(QDPLL * qdpll, int low, int high)
{
    int len = high - low + 1;
    for (int i = (len / 2) - 1; i >= 0; i--) {
        qdpll_adjust(qdpll, i, len);
    }

    for (int i = 0; i < len; i++) 
    //for (int i = len - 1; i > 0; i--) 
    {
        qdpll_swap(qdpll, 0, len - 1 - i);
        qdpll_adjust(qdpll, 0, len - 1 - i);
    }
}

static void find_undeletable_lbd(QDPLL* qdpll, ConstraintList* constraints, int num)
{
    Constraint* c, * prev;
    int order = constraints->cnt, k=0;//顺序
    //printf("%d  %d\n",constraints->cnt, constraints->last - constraints->first);
    for (c = constraints->last; c; c = prev)
    {
        if (c->glue == 0)  set_learnt_constraint_glue(qdpll, c);//计算lbd值，glue在create_constraint中被初始化为0
        if (c->glue < 3)//LBD大于2的不要
        {
            qdpll->undeletable_array[k].score = c->glue;
            qdpll->undeletable_array[k].order = order;
            k++;
        }
        order--;
        prev = c->link.prev;
    }
    qdpll->num_undeletable = k;

    //排序
    qdpll_heapsort(qdpll, 0, k-1);

    num = walksat_min(k, num);
    for (int i = 0; i < num; i++)
    {
        order = qdpll->undeletable_array[i].order;
        qdpll->undeletable[order] = 1;
    }
}

static void find_undeletable_variable_activity(QDPLL* qdpll, ConstraintList* constraints, int num)
{
    Constraint* c, * prev;
    int order = constraints->cnt, k = 0;//顺序
    //printf("%d  %d\n",constraints->cnt, constraints->last - constraints->first);
    for (c = constraints->last; c; c = prev)
    {
        set_learnt_constraint_activity(qdpll, c);//计算平均活跃度值
        qdpll->undeletable_array[k].score = -1 * c->avg_activity;
        //qdpll->undeletable_array[k].score = 1 * c->avg_activity;
        qdpll->undeletable_array[k].order = order;
        k++;
        order--;
...
static void find_undeletable_clause_activity(QDPLL* qdpll, ConstraintList* constraints, int num)
{
    Constraint* c, * prev;
    int order = constraints->cnt, k = 0;//顺序
    //printf("%d  %d\n",constraints->cnt, constraints->last - constraints->first);
    for (c = constraints->last; c; c = prev)
    {
        qdpll->undeletable_array[k].score = c->activity;
        qdpll->undeletable_array[k].order = order;
        k++;
        order--;
        prev = c->link.prev;
    ...
    for (int i = 0; i < k; i++)
    {
        printf("order=%.5d    score=%.10lf    constraints->cnt=%d\n", qdpll->undeletable_array[i].order, qdpll->undeletable_array[i].score, constraints->cnt);
        if (i != k - 1)
        {
            if (qdpll->undeletable_array[i].score > qdpll->undeletable_array[i + 1].score)
            {
                printf("qdpll->undeletable_array[i].score > qdpll->undeletable_array[i + 1].scor\n");
                exit(0);
            }
        }
    }
...
static void find_undeletable_scope(QDPLL* qdpll, ConstraintList* constraints, int num)
{
    Constraint* c, * prev;
    int order = constraints->cnt, k = 0;//顺序
    //printf("%d  %d\n",constraints->cnt, constraints->last - constraints->first);
    //int fid = qdpll->state.forced_assignment.var->id;
    //Var* var = LIT2VARPTR(qdpll->pcnf.vars, fid);
    //double fscope = var->scope->nesting;
    for (c = constraints->last; c; c = prev)
    {
        if(c->avg_scope < 0.000001)  set_learnt_constraint_scope(qdpll, c);//计算平均活跃度值
        //qdpll->undeletable_array[k].score = my_fabs(fscope - c->avg_scope);
        qdpll->undeletable_array[k].score = c->avg_scope;
        //qdpll->undeletable_array[k].score = (int)c->avg_scope;
        //qdpll->undeletable_array[k].score = -1 * c->avg_scope;
        //printf("fscope = %.2lf     c->avg_scope = %.2lf\n", fscope, c->avg_scope);
        //qdpll->undeletable_array[k].score = 1 * c->avg_activity;
        qdpll->undeletable_array[k].order = order;
        k++;
        order--;
    ...
    for (int i = 0; i < k; i++)
    {
        printf("order=%d    score=%.2lf   -----\n", qdpll->undeletable_array[i].order, qdpll->undeletable_array[i].score);
    }*/
    //exit(0);
...
{
  //先判断不能删除的子句
    int _num, res_num = constraints->cnt - try_delete;//剩余学习子句或cube个数
    if (type == QDPLL_QTYPE_EXISTS)  _num = qdpll->state.clause_resizes;
    else  _num = qdpll->state.cube_resizes;
    int iter = 20;
    /*
    if (_num % iter == 0 && _num > 0)
    {
        if (qdpll->use_clause_cube_delete == 1)
        {
            double t1 = clock();
            //（1）按照LBD进行排序
            double ratio_lbd = 1;
            //find_undeletable_lbd(qdpll, constraints, constraints->cnt * ratio_lbd);

            //(2)按照子句中变量活跃度的平均值排序
            double ratio_variable_activity = 0.5;
            find_undeletable_variable_activity(qdpll, constraints, constraints->cnt * ratio_variable_activity);

            //(3)按照量词层数，越接近回溯的量词层越好
            double ratio_scope = 1;
            //find_undeletable_scope(qdpll, constraints, constraints->cnt * ratio_scope);

            //(4)按照子句活跃度排序
            double ratio_clause_activity = 1;
            //find_undeletable_clause_activity(qdpll, constraints, constraints->cnt * ratio_clause_activity);

            double t2 = clock();
            qdpll->sort_time += (t2 - t1) / CLOCKS_PER_SEC;

            if (qdpll->sort_time > 10.0)
            {
                qdpll->use_clause_cube_delete = 0;//超过10s，不再使用
            }
        }
    }*/
    /*
    for (int i = 0; i <= 1e6; i++)
    {
        if (qdpll->undeletable[i] != 0)
        {
            printf("qdpll->undeletable[%d] != 0\n",i);
            exit(0);
        }
    }*/

    
    if (qdpll->use_clause_cube_delete_after_restart == 1 && qdpll->use_clause_cube_delete == 1)
    {
        qdpll->delete_mode = rand() % 100;//学习子句删除模式 
        /*
        printf("%d  ", qdpll->delete_mode);
        if (qdpll->delete_mode >= 10)
        {
            printf(">>>\n");
        }
        else {
        ...
        double t1 = clock();
        //res_num /= 2;
        if (qdpll->delete_mode < 2)
        {
            //(3)按照量词层数，越接近回溯的量词层越好
            //double ratio_scope = 1;
            //find_undeletable_scope(qdpll, constraints, res_num * ratio_scope);
            find_undeletable_scope(qdpll, constraints, res_num);
            qdpll->delete_num++;
        }
        else if (qdpll->delete_mode < 6)
        {
            //（1）按照LBD进行排序
            //double ratio_lbd = 1;
            //find_undeletable_lbd(qdpll, constraints, res_num * ratio_lbd);
            find_undeletable_lbd(qdpll, constraints, res_num);
            qdpll->delete_num++;
        }
        else if (qdpll->delete_mode < 10)
        {
            //(4)按照子句活跃度排序
            //double ratio_clause_activity = 1;
            //find_undeletable_clause_activity(qdpll, constraints, res_num * ratio_clause_activity);
            find_undeletable_clause_activity(qdpll, constraints, res_num);
            qdpll->delete_num++;
        }/*
        else if (qdpll->delete_mode < 20)
        {
            //(2)按照子句中变量活跃度的平均值排序
            //double ratio_variable_activity = 1;
            //find_undeletable_variable_activity(qdpll, constraints, res_num * ratio_variable_activity);
            find_undeletable_variable_activity(qdpll, constraints, res_num);
            qdpll->delete_num++;
        }*/
        ...
        {
            qdpll->use_clause_cube_delete = 0;//超过10s，不再使用
            qdpll->use_clause_cube_delete_after_restart = 0;
        }
    }

    qdpll->undeletable[1] = 1;//刚刚学习到的子句或cube，不可以被删除
  ...
  /* Parameter 'try_delete == UINT_MAX' if this function is called to discard
    ...
    int num = constraints->cnt;

  unsigned int del = 0;
  Constraint *c;
  assert (del < try_delete);
  int num_is_reason = 0, num_is_watched = 0, num_all = 0, num_delete = 0;
  const int no_spure_literals = qdpll->options.no_spure_literals;
  Constraint *prev, *result_constraint = qdpll->result_constraint;
  //bool fff = 0;
  int order = constraints->cnt;
  /*
  for (int ii = 0; ii <= num; ii++)
  {
      if (qdpll->undeletable[ii] == 1)  xxx++;
  }
  if (xxx != res_num && (qdpll->delete_mode < 0 || qdpll->delete_mode > 54))
  {
      printf("xxx != res_num  xxx=%d  res_num=%d    qdpll->delete_mode=%d\n",xxx,res_num, qdpll->delete_mode);
      exit(0);
  }*/
  for (c = constraints->last; c && (del < try_delete); c = prev)
    {
      ...
      */
      //num_all++;
      //if (c->is_reason)  num_is_reason++;
      //if (c->is_watched)  num_is_watched++;
      assert (c->is_cube || c->learnt);
      prev = c->link.prev;

      //c->is_watched>0标志着子句c被至少一个文字监视着，但不是每一个子句都有文字监视着吗？？？
      //c->is_reason：
  ...
  //printf("qdpll->state.lclauses_size=%d  qdpll->pcnf.learnt_clauses.cnt=%d\n", qdpll->state.lclauses_size, qdpll->pcnf.learnt_clauses.cnt);
  //printf("qdpll->state.lcubes_size=%d  qdpll->pcnf.learnt_cubes.cn=%d\n", qdpll->state.lcubes_size, qdpll->pcnf.learnt_cubes.cnt);
  //if (type == QDPLL_QTYPE_EXISTS)  printf("clause: ");
  //else  if (type == QDPLL_QTYPE_FORALL)  printf("cube: ");
  //printf("try_delete=%d  del=%d  num_all=%d  num_is_reason=%d  num_is_watched=%d\n", try_delete, del, num_all, num_is_reason, num_is_watched);
  
  for (int i = 0; i <= num; i++)
  {
      qdpll->undeletable[i] = 0;
  }
...
static unsigned int
learnt_constraints_move(QDPLL* qdpll, const QDPLLQuantifierType type)
{
    ConstraintList* constraints;
    if (type == QDPLL_QTYPE_EXISTS)  constraints = &(qdpll->pcnf.learnt_clauses);
    else  constraints = &(qdpll->pcnf.learnt_cubes);
    //先判断不能删除的子句
    int res_num = constraints->cnt;
    ...
    */
    ...
    for (c = constraints->last; c; c = prev)
    {
        ...
        else if (qdpll->undeletable[order] == 2)
        {
            break;
        }
        order--;
...
check_resize_learnt_constraints (QDPLL * qdpll, const QDPLLQuantifierType type)
{
  ...
  if (type == QDPLL_QTYPE_EXISTS)
    ...
    {
      if (!cur_exceeded_soft_max_space && qdpll->pcnf.learnt_cubes.cnt < qdpll->state.lcubes_size)  return;
      constraints = &(qdpll->pcnf.learnt_cubes);
    }
  ...
  //qdpll->num_discarded_clause_cube += del;

  for (int i = 0; i <= qdpll->pcnf.size_vars; i++)
  {
      qdpll->hope[i] /= 2;
  }
...
static void
check_resize_learnt_constraints_after_restart(QDPLL* qdpll, const QDPLLQuantifierType type)
{
    QDPLL_ABORT_QDPLL(qdpll->state.lclauses_size == 0, "Size of learned clauses must be greater than zero!");
    QDPLL_ABORT_QDPLL(qdpll->state.lcubes_size == 0, "Size of learned cubes must be greater than zero!");
    ...
    if (type == QDPLL_QTYPE_EXISTS)  qdpll->state.clause_resizes++;
    else  qdpll->state.cube_resizes++;
...
static int
check_and_restart (QDPLL * qdpll, unsigned int backtrack_level, WalkSAT * walksat)
{
    //return 0;
  if (backtrack_level > 1 && qdpll->state.irestart_dist &&
       ...
       qdpll->state.last_backtracks) >= qdpll->state.irestart_dist)
    {
...
#endif
      qdpll->current_state = 0;//重启不算
      //printf("qdpll->state.irestart_dist=%d   btlevel=%d  backtrack_level=%d  highest_univ=%d\n", qdpll->state.irestart_dist, btlevel, backtrack_level, highest_univ);
      backtrack (qdpll, btlevel);
      /*
      //(1)重启后每间隔一段次数，就使用新删除策略
      qdpll->delete_mode = 101;
      int _num = qdpll->state.num_restarts;
      int _iter = 50;
      if (qdpll->use_clause_cube_delete == 1 && (_num % _iter == 0 && _num > 0))//重启后删除一部分学习子句
      {
          //printf("---\n"); 
          qdpll->use_clause_cube_delete_after_restart = 1;
          check_resize_learnt_constraints_after_restart(qdpll, QDPLL_QTYPE_EXISTS);
          check_resize_learnt_constraints_after_restart(qdpll, QDPLL_QTYPE_FORALL);
          qdpll->delete_num++;
      }
      qdpll->delete_mode = 101;
      qdpll->use_clause_cube_delete_after_restart = 0;
      */
      ...
      //(2)重启后，就使用新删除策略
      qdpll->delete_mode = 101;
     if (qdpll->use_clause_cube_delete_after_restart == 1)//重启后删除一部分学习子句
      {
          qdpll->delete_mode = rand() % 100;//学习子句删除模式
          check_resize_learnt_constraints_after_restart(qdpll, QDPLL_QTYPE_EXISTS);
          qdpll->delete_mode = rand() % 100;//学习子句删除模式
      ...
      */
      ...
      //(3)重启到顶层时，使用新删除策略
      qdpll->delete_mode = 101;
      if (qdpll->use_clause_cube_delete_after_restart == 1 && btlevel == 1)
      {
          qdpll->delete_mode = rand() % 100;//学习子句删除模式
          check_resize_learnt_constraints_after_restart(qdpll, QDPLL_QTYPE_EXISTS);
          qdpll->delete_mode = rand() % 100;//学习子句删除模式
      ...
      */
      ...
      /*
      //(4)重启到0层后每间隔一段次数，就使用新删除策略
      int _num = qdpll->num_backtrack_zero;
      int _iter = 10;
      if (qdpll->use_clause_cube_delete == 1 && btlevel == 1 && (_num % _iter == 0 && _num > 0))//重启后删除一部分学习子句
      {
          //printf("---\n"); 
          qdpll->use_clause_cube_delete_after_restart = 1;
          check_resize_learnt_constraints_after_restart(qdpll, QDPLL_QTYPE_EXISTS);
      ...
      */
      ...
      /*
      //(5)重启到0层后，就使用新删除策略
      if (qdpll->use_clause_cube_delete == 1 && btlevel == 1)//重启后删除一部分学习子句
      {
          //printf("---\n"); 
          qdpll->use_clause_cube_delete_after_restart = 1;
          check_resize_learnt_constraints_after_restart(qdpll, QDPLL_QTYPE_EXISTS);
      ...
      */
      ...
      /*
      //(1)重启后每间隔一段次数，就使用新移动策略
      int _num = qdpll->state.num_restarts;
      int _iter = 10;
      if (qdpll->clause_move == 1 && (_num % _iter == 0 && _num > 0))
      {
          learnt_constraints_move(qdpll, QDPLL_QTYPE_EXISTS);
          learnt_constraints_move(qdpll, QDPLL_QTYPE_FORALL);
          qdpll->delete_num++;
      }
      */
      ...
      //qdpll->branch_in_forall = 0;

      if (btlevel == 1)
      {
          qdpll->call_ls_num = 0;
          qdpll->ls_r = 0;
          qdpll->num_backtrack_zero++;
          //qdpll->branch_in_forall = qdpll->first_branch_in_forall;
          qdpll->branch_in_forall = 0;
          
          if (walksat->use_hash && 1)
          {
              for (int i = 0; i <= walksat->size_hash; i++)  walksat->hash[i] = 0;
              
          }
      }
      else {
          //qdpll->branch_in_forall = 1;
          //qdpll->call_ls_num = 0;
          //qdpll->ls_r = 0;
          //qdpll->branch_in_forall = qdpll->first_branch_in_forall;
          qdpll->branch_in_forall = 0;
      ...
      //if (qdpll->use_individual_heuristic == 1 && 1 && btlevel == 1)

      if (qdpll->use_individual_heuristic == 1 && 1)
      {
          
          //if(btlevel != backtrack_level)  qdpll->skip_variable = 0;

          if (qdpll->only_phase_saving == 0)
          {
              for (int i = 0; i <= walksat->num_vars; i++)
              {
                  qdpll->_polarity[i] = 2;
              }
              ...
              {
                  v = VARID2VARPTR(qdpll->pcnf.vars, *vp);
                  qdpll->_polarity[v->id] = (int)v->assignment;
              }
          }
      }
      //if (btlevel != backtrack_level)  qdpll->skip_variable = 0;
...
void modify_qsscore(QDPLL* qdpll, Constraint* c, QDPLLQuantifierType type)
{
    if (c == 0)
    {
        return;
    ...
    {
        v = LIT2VARPTR(vars, *p);
        if (type != v->scope->type)  continue;//cube只看任意文字，clause只看存在文字
        bp = *p;//为bp加分
        if (bp > 0)
        {
            qdpll->hope[bp]++;
...
qbcp_qbce_check_blocked_clauses_after_clause_addition (QDPLL *qdpll)
{
  ...
  ConstraintPtrStack _new;
  QDPLL_INIT_STACK (_new);
      ...
      if (c->qbcp_qbce_blocked)
        {
          ...
          assert (c->qbcp_qbce_blocking_lit);
          QDPLL_PUSH_STACK (qdpll->mm, _new, c);
        }
      else
        assert (!c->qbcp_qbce_blocking_lit);
    }
  ...
  qdpll->qbcp_qbce_blocked_clauses.start[0] = _new;
  ...
  while (unsat_branch_clause->lits <= last_irred_exist_lit)
    {
      ...
      assert(ccadical_failed(qdpll->trivial_falsity_solver, -last_lit));
          ...
          for (p2 = e + unsat_branch_clause->size_lits - 1; e <= p2; p2--)
            {
              ...
              if (lit2 != lit && 
                  ...
                  ccadical_assume (qdpll->trivial_falsity_solver, -lit2);
            }
#if COMPUTE_STATS
          qdpll->stats.trivial_falsity_remove_inner_existentials_sat_calls++;
#endif
          ccadical_limit(qdpll->trivial_falsity_solver, "decisions",
              qdpll->options.trivial_falsity_decision_limit > 0 ? (int)qdpll->options.trivial_falsity_decision_limit : -1);
          int sat_result = ccadical_sat(qdpll->trivial_falsity_solver);
          /*
          int sat_result = ccadical_sat (qdpll->trivial_falsity_solver,
                                        qdpll->options.trivial_falsity_decision_limit > 0 ? (int) qdpll->options.trivial_falsity_decision_limit : -1);*/
              ...
              for (p3 = last_irred_exist_lit; e <= p3; p3--)
                {
                  ...
                  if (!ccadical_failed(qdpll->trivial_falsity_solver, -lit3))
                    {
...
/* Given a clause learned by trivial falsity, the assignment defined by that
   ...
   variables. 
   对于通过 “平凡假”（trivial falsity）学习到的子句而言，该子句所定义的赋值不一定是 QCDCL 赋值。
   也就是说，变量的赋值顺序不一定遵循量词前缀顺序。但需注意，即便我们跳过了外层的存在量词变量和全称量词变量，在该赋值下的 QBF 公式仍必然是不可满足的（UNSAT） 。*/
static void
...
const int* ccadical_failed_assumptions(QDPLL * qdpll)
{
    int cnt = 0, v;
    VarID* vp, * ve;
    Var* _v;
    for (vp = qdpll->assigned_vars, ve = qdpll->assigned_vars_top; vp < ve; vp++)
    {
        _v = VARID2VARPTR(qdpll->pcnf.vars, *vp);
        assert(QDPLL_VAR_ASSIGNED(_v));
        v = (int)*vp;
        if (QDPLL_VAR_ASSIGNED_FALSE(_v))
        {
            v *= -1;
        }
        if (ccadical_failed(qdpll->trivial_falsity_solver, v))
        {
            cnt++;
        }
    }
    int* failed_assumptions = (int*)malloc((cnt + 1) * sizeof(int));
    cnt = 0;
    for (vp = qdpll->assigned_vars, ve = qdpll->assigned_vars_top; vp < ve; vp++)
    {
        _v = VARID2VARPTR(qdpll->pcnf.vars, *vp);
        ...
        if (ccadical_failed(qdpll->trivial_falsity_solver, v))
        {
            failed_assumptions[cnt++] = v;
        }
    }
    failed_assumptions[cnt] = 0;
...
handle_trivial_falsity (QDPLL *qdpll)
{
  ...
  cnt = 0;
  failed_assumptions = ccadical_failed_assumptions (qdpll);
...
trivial_falsity_set_up_sat_solver (QDPLL *qdpll)
{
  ...
  qdpll->trivial_falsity_solver = ccadical_init();
  ...
  for (c = qdpll->pcnf.clauses.first; c; c = c->link.next)
    {
      LitID *p, *e;
      for (p = c->lits, e = p + c->num_lits; p < e; p++)
          ccadical_add (qdpll->trivial_falsity_solver, *p);
      ccadical_add (qdpll->trivial_falsity_solver, 0);
    }
}
...
trivial_truth_set_up_sat_solver (QDPLL *qdpll)
{
  ...
  qdpll->trivial_truth_solver = ccadical_init();
      ...
      for (p = c->lits, e = p + c->num_lits; p < e; p++)
        {
          ...
          if (QDPLL_VAR_FORALL (v))
            {
              ...
              ccadical_add (qdpll->trivial_truth_solver, lit);
        }
          ...
          ccadical_add (qdpll->trivial_truth_solver, c->trivial_truth_selector);
        ccadical_add (qdpll->trivial_truth_solver, 0);
    }
}
...
void init_walksat(QDPLL* qdpll, WalkSAT* walksat)
{
    //walksat->org_clause_lit[c] = (SAT_lit1*)qdpll_malloc(mm, (walksat->clause_lit_count[c] + 1) * sizeof(SAT_lit1));
    QDPLLMemMan* mm = qdpll->mm;
    int c = 0;
    Constraint* _c;
    unsigned int max_c = 0;
    walksat->satisfy_flag = 0;
    for (_c = qdpll->pcnf.clauses.first; _c; _c = _c->link.next)//_c是每一个子句
    {
        c++;
        //cc = _c->id;
        //printf("_c->id = %d    cc=%d\n", _c->id,cc);
        ...
        {
            //printf("_c->id > max_c\n");
            max_c = _c->id;//用于后续开辟walksat->clause_qbf_to_sat数组用
        }
        else {
            //printf("_c->id <= max_c\n");
        }
    ...
    //printf("max_c = %d    c = %d\n", max_c, c);
    qdpll->pcnf.size_clause = c;
    int num_var = qdpll->pcnf.size_vars - 1, num_clause = qdpll->pcnf.size_clause;
    //printf("* %d  %d\n", qdpll->pcnf.size_vars, qdpll->pcnf.size_clause);
    //walksat->var_lit_count = new int[10];
    walksat->var_lit_count = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->var_lit_count, 0, (num_var + 1) * sizeof(int));

    walksat->var_type = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->var_type, 0, (num_var + 1) * sizeof(int));

    walksat->clause_lit_count = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->clause_lit_count, 0, (num_clause + 1) * sizeof(int));

    walksat->clause_var_count = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->clause_var_count, 0, (num_clause + 1) * sizeof(int));

    walksat->clause_sat_to_qbf = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->clause_sat_to_qbf, -1, (num_clause + 1) * sizeof(int));

    walksat->clause_qbf_to_sat = (int*)malloc((max_c + 1) * sizeof(int));
    memset(walksat->clause_qbf_to_sat, -1, (max_c + 1) * sizeof(int));

    walksat->org_clause_lit_count = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->org_clause_lit_count, 0, (num_clause + 1) * sizeof(int));

    walksat->score = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->score, 0, (num_var + 1) * sizeof(int));

    //printf("111\n");

    walksat->time_stamp = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->time_stamp, 0, (num_var + 1) * sizeof(int));

    walksat->conf_change = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->conf_change, 0, (num_var + 1) * sizeof(int));

    walksat->freq = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->freq, 0, (num_var + 1) * sizeof(int));

    walksat->freq_sat_solution = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->freq_sat_solution, 0, (num_var + 1) * sizeof(int));

    walksat->var_neighbor_count = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->var_neighbor_count, 0, (num_var + 1) * sizeof(int));

    walksat->fix = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->fix, 0, (num_var + 1) * sizeof(int));

    walksat->dpll_fix = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->dpll_fix, 0, (num_var + 1) * sizeof(int));

    walksat->clause_weight = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->clause_weight, 0, (num_clause + 1) * sizeof(int));

    walksat->sat_count = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->sat_count, 0, (num_clause + 1) * sizeof(int));

    //printf("222\n");

    walksat->sat_var = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->sat_var, 0, (num_clause + 1) * sizeof(int));

    walksat->unsat_stack = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->unsat_stack, 0, (num_clause + 1) * sizeof(int));

    walksat->index_in_unsat_stack = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->index_in_unsat_stack, 0, (num_clause + 1) * sizeof(int));

    walksat->unsatvar_stack = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->unsatvar_stack, 0, (num_var + 1) * sizeof(int));

    walksat->index_in_unsatvar_stack = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->index_in_unsatvar_stack, 0, (num_var + 1) * sizeof(int));

    walksat->unsat_app_count = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->unsat_app_count, 0, (num_var + 1) * sizeof(int));

    walksat->goodvar_stack = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->goodvar_stack, 0, (num_var + 1) * sizeof(int));

    walksat->already_in_goodvar_stack = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->already_in_goodvar_stack, 0, (num_var + 1) * sizeof(int));

    walksat->unitclause_queue = (SAT_lit*)malloc((num_clause + 1) * sizeof(SAT_lit));
    //memset(walksat->unitclause_queue, 0, (num_clause + 1) * sizeof(SAT_lit1));

    //printf("333\n");

    walksat->clause_delete = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->clause_delete, 0, (num_clause + 1) * sizeof(int));

    walksat->cur_soln = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->cur_soln, 0, (num_var + 1) * sizeof(int));

    walksat->cur_soln1 = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->cur_soln1, 0, (num_var + 1) * sizeof(int));

    walksat->qbf_org_bcp = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->qbf_org_bcp, 0, (num_var + 1) * sizeof(int));

    walksat->is_hope = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->is_hope, 0, (num_var + 1) * sizeof(int));

    walksat->clause_unsat_count = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->clause_unsat_count, 0, (num_clause + 1) * sizeof(int));

    walksat->clause_unsat_stamp = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->clause_unsat_stamp, 0, (num_clause + 1) * sizeof(int));

    walksat->clause_unsat_count_sort = (SAT_clause*) malloc((num_clause + 1) * sizeof(SAT_clause));
    //memset(walksat->clause_unsat_count_sort, 0, (num_clause + 1) * sizeof(SAT_clause1));

    walksat->num_through_var_decision = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->num_through_var_decision, 0, (num_var + 1) * sizeof(int));

    walksat->num_through_var_decision_sat = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->num_through_var_decision_sat, 0, (num_var + 1) * sizeof(int));

    //printf("444\n");

    walksat->had_vis = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->had_vis, 0, (num_clause + 1) * sizeof(int));

    walksat->due_to_second_backtrack = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->due_to_second_backtrack, 0, (num_var + 1) * sizeof(int));
    //printf("444.0\n");

    walksat->wr = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->wr, 0, (num_var + 1) * sizeof(int));

    walksat->in_wr = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->in_wr, 0, (num_var + 1) * sizeof(int));

    walksat->var_neighbor = (int**)malloc((num_var + 1) * sizeof(int*));
    //memset(walksat->var_neighbor, 0, (num_var + 1) * sizeof(int*));
    //printf("444.1\n");

    walksat->var_lit = (SAT_lit**)malloc((num_var + 1) * sizeof(SAT_lit*));
    //memset(walksat->var_lit, 0, (num_var + 1) * sizeof(SAT_lit1*));

    walksat->clause_lit = (SAT_lit**)malloc((num_clause + 1) * sizeof(SAT_lit*));
    //memset(walksat->clause_lit, 0, (num_clause + 1) * sizeof(SAT_lit1*));
    //printf("444.1.5\n");

    walksat->org_clause_lit = (SAT_lit**)malloc((num_clause + 1) * sizeof(SAT_lit*));
    //memset(walksat->org_clause_lit, 0, (num_clause + 1) * sizeof(SAT_lit1*));

    walksat->qsscore = (int**)malloc(3 * sizeof(int*));
    //memset(walksat->qsscore, 0, 3 * sizeof(int*));
    //printf("444.2\n");

    walksat->qsscore[0] = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->qsscore[0], 0, (num_var + 1) * sizeof(int));

    walksat->qsscore[1] = (int*)malloc((num_var + 1) * sizeof(int));
    memset(walksat->qsscore[1], 0, (num_var + 1) * sizeof(int));

    //printf("444.3\n");

    walksat->clause_to_variable_activity = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->clause_to_variable_activity, 0, (num_clause + 1) * sizeof(int));

    //printf("444.4\n");

    walksat->temp_list = (int*)malloc((num_var + 1) * sizeof(int));

    //printf("444.5\n");

    walksat->vc = (int*)malloc((num_var + 1) * sizeof(int));

    walksat->vc_v = (int*)malloc((num_var + 1) * sizeof(int));

    walksat->num_vc = 0;

    walksat->nesting = (int*)malloc((num_var + 1) * sizeof(int));

    //walksat->is_collected = (int*)malloc((num_var + 1) * sizeof(int));
    //memset(walksat->is_collected, 0, (num_var + 1) * sizeof(int));


    walksat->record_priority = (int*)malloc((num_clause + 1) * sizeof(int));
    memset(walksat->record_priority, 0, (num_clause + 1) * sizeof(int));

    walksat->hash = (int*)malloc((walksat->size_hash + 1) * sizeof(int));
    memset(walksat->hash, 0, (walksat->size_hash + 1) * sizeof(int));
    for (int i = 0; i < walksat->size_hash; i++)  walksat->hash[i] = 0;
    //printf("555\n");

    //qdpll_free (mm, qdpll->assigned_vars, size_assigned_vars(qdpll) * sizeof(VarID));
}

void init_walksat1(QDPLL* qdpll, WalkSAT* walksat)
{
    //walksat->org_clause_lit[c] = (SAT_lit1*)qdpll_malloc(mm, (walksat->clause_lit_count[c] + 1) * sizeof(SAT_lit1));
    QDPLLMemMan* mm = qdpll->mm;
    int c = 0;
    ...
    // ===================== 第一个for循环：处理所有长度为 num_var+1 的数组 赋值为 0 =====================
    int i;
    for (i = 0; i <= num_var; i++)
    {
        walksat->var_lit_count[i] = 0;
        walksat->var_type[i] = 0;
        walksat->score[i] = 0;
        walksat->time_stamp[i] = 0;
        walksat->conf_change[i] = 0;
        walksat->freq[i] = 0;
        walksat->freq_sat_solution[i] = 0;
        walksat->var_neighbor_count[i] = 0;
        walksat->fix[i] = 0;
        walksat->dpll_fix[i] = 0;
        walksat->unsatvar_stack[i] = 0;
        walksat->index_in_unsatvar_stack[i] = 0;
        walksat->unsat_app_count[i] = 0;
        walksat->goodvar_stack[i] = 0;
        walksat->already_in_goodvar_stack[i] = 0;
        walksat->cur_soln[i] = 0;
        walksat->cur_soln1[i] = 0;
        walksat->qbf_org_bcp[i] = 0;
        walksat->is_hope[i] = 0;
        walksat->num_through_var_decision[i] = 0;
        walksat->num_through_var_decision_sat[i] = 0;
        walksat->due_to_second_backtrack[i] = 0;
        walksat->wr[i] = 0;
        walksat->in_wr[i] = 0;
        walksat->qsscore[0][i] = 0;
        walksat->qsscore[1][i] = 0;
    }

    // ===================== 第二个for循环：处理所有长度为 num_clause+1 的数组 =====================
    int j;
    for (j = 0; j <= num_clause; j++)
    {
        walksat->clause_lit_count[j] = 0;
        walksat->clause_var_count[j] = 0;
        walksat->clause_sat_to_qbf[j] = -1;  // 原memset赋值-1
        walksat->org_clause_lit_count[j] = 0;
        walksat->clause_weight[j] = 0;
        walksat->sat_count[j] = 0;
        walksat->sat_var[j] = 0;
        walksat->unsat_stack[j] = 0;
        walksat->index_in_unsat_stack[j] = 0;
        walksat->clause_delete[j] = 0;
        walksat->clause_unsat_count[j] = 0;
        walksat->clause_unsat_stamp[j] = 0;
        walksat->had_vis[j] = 0;
        walksat->clause_to_variable_activity[j] = 0;
        walksat->record_priority[j] = 0;
    }

    // 单独处理 clause_qbf_to_sat 数组 (长度是 max_c+1，赋值-1)
    int k;
    for (k = 0; k <= max_c; k++)
    {
        walksat->clause_qbf_to_sat[k] = -1;
    }
...
void free_walksat(QDPLL* qdpll, WalkSAT* walksat)
{
    QDPLLMemMan* mm = qdpll->mm;
    //printf("000\n");
    return;
    ...
    //walksat->var_neighbor = (int**)qdpll_malloc(mm, (num_var + 1) * sizeof(int*));
    //walksat->var_lit = (SAT_lit1**)qdpll_malloc(mm, (num_var + 1) * sizeof(SAT_lit1*));
    //walksat->clause_lit = (SAT_lit1**)qdpll_malloc(mm, (num_clause + 1) * sizeof(SAT_lit1*));
    //walksat->org_clause_lit = (SAT_lit1**)qdpll_malloc(mm, (num_clause + 1) * sizeof(SAT_lit1*));
    printf("111\n");
    ...
    for (_c = qdpll->pcnf.clauses.first; _c; _c = _c->link.next)//_c是每一个子句
    {
        if (_c->id > max_c)  max_c = _c->id;
    }
    /*
    for (int i = 1; i <= num_var; i++)
    {
        if (walksat->var_neighbor_count[i] > 0)
        {
            qdpll_free(mm, walksat->var_neighbor[i], walksat->var_neighbor_count[i] * sizeof(int));
            //walksat->var_neighbor[i] = NULL;
        }
        if (walksat->var_lit_count[i] > 0)
        {
            qdpll_free(mm, walksat->var_lit[i], walksat->var_lit_count[i] * sizeof(SAT_lit));
            //walksat->var_lit[i] = NULL;
        }
    }*/

    if (walksat->clause_lit[0])  printf("!walksat->clause_lit[0]\n");
    printf("%d\n", walksat->clause_lit[0]);
    printf("000\n");
    qdpll_free(mm, walksat->clause_lit[0], (walksat->clause_lit_count[0] + 1) * sizeof(SAT_lit));
    printf("111\n");

    for (int i = 0; i < num_clause; i++)
    {
        if (walksat->clause_lit_count[i] > 0)
        {
            
            printf("i = %d    walksat->clause_lit_count[i] = %d    walksat->clause_lit[i]=%d  num_clause=%d  delete=%d\n",
                i, walksat->clause_lit_count[i], walksat->clause_lit[i], num_clause,walksat->clause_delete[i]);
            //if (i <= 4)  continue;
            printf("i=%d  walksat->org_clause_lit_count=%d\n", i, walksat->org_clause_lit_count[i]);
            for (int k = 0; k <= walksat->clause_lit_count[i]; k++)
            {
                printf("k=%d  lit=%d\n", k, walksat->clause_lit[i][k].var_num);
            }
            printf("i=%d---\n", i);
            qdpll_free(mm, walksat->clause_lit[i], (walksat->clause_lit_count[i] + 1) * sizeof(SAT_lit));
            //walksat->clause_lit[i] = NULL;
        }
        if (walksat->org_clause_lit_count[i] > 0 && walksat->unitclause_queue_end_pointer > 0)
        {
            //printf("* i = %d\n", i);
            qdpll_free(mm, walksat->org_clause_lit[i], (walksat->org_clause_lit_count[i] + 1) * sizeof(SAT_lit));
            //walksat->org_clause_lit[i] = NULL;
        }
    }
    printf("333\n");

    // 释放二维指针的一级指针
    qdpll_free(mm, walksat->var_neighbor, (num_var + 1) * sizeof(int*));
    //walksat->var_neighbor = NULL;

    qdpll_free(mm, walksat->var_lit, (num_var + 1) * sizeof(SAT_lit*));
    //walksat->var_lit = NULL;

    qdpll_free(mm, walksat->clause_lit, (num_clause + 1) * sizeof(SAT_lit*));
    //walksat->clause_lit = NULL;

    qdpll_free(mm, walksat->org_clause_lit, (num_clause + 1) * sizeof(SAT_lit*));
    //walksat->org_clause_lit = NULL;

    qdpll_free(mm, walksat->clause_sat_to_qbf, (num_clause + 1) * sizeof(int));
    qdpll_free(mm, walksat->clause_qbf_to_sat, (max_c + 1) * sizeof(int));

    printf("444\n");

    qdpll_free(mm, walksat->var_lit_count, (num_var + 1) * sizeof(int));
    walksat->var_lit_count = NULL;

    qdpll_free(mm, walksat->var_type, (num_var + 1) * sizeof(int));
    walksat->var_type = NULL;

    qdpll_free(mm, walksat->clause_lit_count, (num_clause + 1) * sizeof(int));
    walksat->clause_lit_count = NULL;

    qdpll_free(mm, walksat->clause_var_count, (num_clause + 1) * sizeof(int));
    walksat->clause_var_count = NULL;

    qdpll_free(mm, walksat->org_clause_lit_count, (num_clause + 1) * sizeof(int));
    walksat->org_clause_lit_count = NULL;

    qdpll_free(mm, walksat->score, (num_var + 1) * sizeof(int));
    walksat->score = NULL;

    qdpll_free(mm, walksat->time_stamp, (num_var + 1) * sizeof(int));
    walksat->time_stamp = NULL;

    qdpll_free(mm, walksat->conf_change, (num_var + 1) * sizeof(int));
    walksat->conf_change = NULL;

    qdpll_free(mm, walksat->freq, (num_var + 1) * sizeof(int));
    walksat->freq = NULL;

    printf("555\n");

    qdpll_free(mm, walksat->freq_sat_solution, (num_var + 1) * sizeof(int));
    walksat->freq_sat_solution = NULL;

    qdpll_free(mm, walksat->var_neighbor_count, (num_var + 1) * sizeof(int));
    walksat->var_neighbor_count = NULL;

    qdpll_free(mm, walksat->fix, (num_var + 1) * sizeof(int));
    walksat->fix = NULL;

    qdpll_free(mm, walksat->dpll_fix, (num_var + 1) * sizeof(int));
    walksat->dpll_fix = NULL;

    qdpll_free(mm, walksat->clause_weight, (num_clause + 1) * sizeof(int));
    walksat->clause_weight = NULL;

    qdpll_free(mm, walksat->sat_count, (num_clause + 1) * sizeof(int));
    walksat->sat_count = NULL;

    qdpll_free(mm, walksat->sat_var, (num_clause + 1) * sizeof(int));
    walksat->sat_var = NULL;

    qdpll_free(mm, walksat->unsat_stack, (num_clause + 1) * sizeof(int));
    walksat->unsat_stack = NULL;

    qdpll_free(mm, walksat->index_in_unsat_stack, (num_clause + 1) * sizeof(int));
    walksat->index_in_unsat_stack = NULL;

    printf("666\n");

    qdpll_free(mm, walksat->unsatvar_stack, (num_var + 1) * sizeof(int));
    walksat->unsatvar_stack = NULL;

    qdpll_free(mm, walksat->index_in_unsatvar_stack, (num_var + 1) * sizeof(int));
    walksat->index_in_unsatvar_stack = NULL;

    qdpll_free(mm, walksat->unsat_app_count, (num_var + 1) * sizeof(int));
    walksat->unsat_app_count = NULL;

    qdpll_free(mm, walksat->goodvar_stack, (num_var + 1) * sizeof(int));
    walksat->goodvar_stack = NULL;

    qdpll_free(mm, walksat->already_in_goodvar_stack, (num_var + 1) * sizeof(int));
    walksat->already_in_goodvar_stack = NULL;

    qdpll_free(mm, walksat->unitclause_queue, (num_clause + 1) * sizeof(SAT_lit));
    walksat->unitclause_queue = NULL;

    qdpll_free(mm, walksat->clause_delete, (num_clause + 1) * sizeof(int));
    walksat->clause_delete = NULL;

    qdpll_free(mm, walksat->cur_soln, (num_var + 1) * sizeof(int));
    walksat->cur_soln = NULL;

    qdpll_free(mm, walksat->cur_soln1, (num_var + 1) * sizeof(int));
    walksat->cur_soln1 = NULL;

    qdpll_free(mm, walksat->qbf_org_bcp, (num_var + 1) * sizeof(int));
    walksat->qbf_org_bcp = NULL;

    printf("777\n");

    qdpll_free(mm, walksat->is_hope, (num_var + 1) * sizeof(int));
    walksat->is_hope = NULL;

    qdpll_free(mm, walksat->clause_unsat_count, (num_clause + 1) * sizeof(int));
    walksat->clause_unsat_count = NULL;

    qdpll_free(mm, walksat->clause_unsat_stamp, (num_clause + 1) * sizeof(int));
    walksat->clause_unsat_stamp = NULL;

    qdpll_free(mm, walksat->clause_unsat_count_sort, (num_clause + 1) * sizeof(SAT_clause));
    walksat->clause_unsat_count_sort = NULL;

    qdpll_free(mm, walksat->num_through_var_decision, (num_var + 1) * sizeof(int));
    walksat->num_through_var_decision = NULL;

    qdpll_free(mm, walksat->num_through_var_decision_sat, (num_var + 1) * sizeof(int));
    walksat->num_through_var_decision_sat = NULL;

    qdpll_free(mm, walksat->had_vis, (num_clause + 1) * sizeof(int));
    walksat->had_vis = NULL;

    qdpll_free(mm, walksat->due_to_second_backtrack, (num_var + 1) * sizeof(int));
    walksat->due_to_second_backtrack = NULL;

    qdpll_free(mm, walksat->wr, (num_var + 1) * sizeof(int));
    walksat->wr = NULL;

    qdpll_free(mm, walksat->in_wr, (num_var + 1) * sizeof(int));
    walksat->in_wr = NULL;

    // qsscore二维数组 先释放子数组 再释放一级数组
    qdpll_free(mm, walksat->qsscore[0], (num_var + 1) * sizeof(int));
    walksat->qsscore[0] = NULL;

    qdpll_free(mm, walksat->qsscore[1], (num_var + 1) * sizeof(int));
    walksat->qsscore[1] = NULL;

    qdpll_free(mm, walksat->qsscore, 3 * sizeof(int*));
    walksat->qsscore = NULL;

    qdpll_free(mm, walksat->clause_to_variable_activity, (num_clause + 1) * sizeof(int));
    qdpll_free(mm, walksat->temp_list, (num_var + 1) * sizeof(int));
    qdpll_free(mm, walksat->vc, (num_var + 1) * sizeof(int));
    qdpll_free(mm, walksat->vc_v, (num_var + 1) * sizeof(int));
    qdpll_free(mm, walksat->record_priority, (num_clause + 1) * sizeof(int));

    // 最后释放WalkSAT结构体本身，保留你的原生写法
    qdpll_free(mm, walksat, sizeof(WalkSAT));
...
void free_walksat1(QDPLL* qdpll, WalkSAT* walksat)
{
    QDPLLMemMan* mm = qdpll->mm;
    //printf("000\n");
    //return;
    ...
    for (int i = 1; i <= num_var; i++)
    {
        if (walksat->var_neighbor_count[i] > 0)
        {
            free(walksat->var_neighbor[i]);
            //walksat->var_neighbor[i] = NULL;
        }
        if (walksat->var_lit_count[i] > 0)
        {
            free(walksat->var_lit[i]);
            //walksat->var_lit[i] = NULL;
        }
    }
    ...
    for (int i = 0; i < num_clause; i++)
    {
        if (walksat->clause_lit_count[i] > 0)
        {
            ...
            printf("i=%d---\n", i);*/
            free(walksat->clause_lit[i]);
            //walksat->clause_lit[i] = NULL;
        }
        if (walksat->org_clause_lit_count[i] > 0 && walksat->unitclause_queue_end_pointer > 0)
        {
            //printf("* i = %d\n", i);
            free(walksat->org_clause_lit[i]);
            //walksat->org_clause_lit[i] = NULL;
        }
    }
    printf("333\n");
    ...
    free(walksat->var_neighbor);
    //walksat->var_neighbor = NULL;

    free(walksat->var_lit);
    //walksat->var_lit = NULL;

    free(walksat->clause_lit);
    //walksat->clause_lit = NULL;

    free(walksat->org_clause_lit);
    //walksat->org_clause_lit = NULL;

    free(walksat->clause_sat_to_qbf);

    free(walksat->clause_qbf_to_sat);

    printf("444\n");

    free(walksat->var_lit_count);
    walksat->var_lit_count = NULL;

    free(walksat->var_type);
    walksat->var_type = NULL;

    free(walksat->clause_lit_count);
    walksat->clause_lit_count = NULL;

    free(walksat->clause_var_count);
    walksat->clause_var_count = NULL;

    free(walksat->org_clause_lit_count);
    walksat->org_clause_lit_count = NULL;

    free(walksat->score);
    walksat->score = NULL;

    free(walksat->time_stamp);
    walksat->time_stamp = NULL;

    free(walksat->conf_change);
    walksat->conf_change = NULL;
    ...
    free(walksat->freq_sat_solution);
    walksat->freq_sat_solution = NULL;

    free(walksat->var_neighbor_count);
    walksat->var_neighbor_count = NULL;

    free(walksat->fix);
    walksat->fix = NULL;

    free(walksat->dpll_fix);
    walksat->dpll_fix = NULL;

    free(walksat->clause_weight);
    walksat->clause_weight = NULL;

    free(walksat->sat_count);
    walksat->sat_count = NULL;

    free(walksat->sat_var);
    walksat->sat_var = NULL;

    free(walksat->unsat_stack);
    walksat->unsat_stack = NULL;

    free(walksat->index_in_unsat_stack);
    walksat->index_in_unsat_stack = NULL;

    printf("666\n");

    free(walksat->unsatvar_stack);
    walksat->unsatvar_stack = NULL;

    free(walksat->index_in_unsatvar_stack);
    walksat->index_in_unsatvar_stack = NULL;

    free(walksat->unsat_app_count);
    walksat->unsat_app_count = NULL;

    free(walksat->goodvar_stack);
    walksat->goodvar_stack = NULL;

    free(walksat->already_in_goodvar_stack);
    walksat->already_in_goodvar_stack = NULL;

    free(walksat->unitclause_queue);
    walksat->unitclause_queue = NULL;

    free(walksat->clause_delete);
    walksat->clause_delete = NULL;

    free(walksat->cur_soln);
    walksat->cur_soln = NULL;

    free(walksat->cur_soln1);
    walksat->cur_soln1 = NULL;

    free(walksat->qbf_org_bcp);
    walksat->qbf_org_bcp = NULL;

    printf("777\n");

    free(walksat->is_hope);
    walksat->is_hope = NULL;

    free(walksat->clause_unsat_count);
    walksat->clause_unsat_count = NULL;

    free(walksat->clause_unsat_stamp);
    walksat->clause_unsat_stamp = NULL;

    free(walksat->clause_unsat_count_sort);
    walksat->clause_unsat_count_sort = NULL;

    free(walksat->num_through_var_decision);
    walksat->num_through_var_decision = NULL;

    free(walksat->num_through_var_decision_sat);
    walksat->num_through_var_decision_sat = NULL;

    free(walksat->had_vis);
    walksat->had_vis = NULL;

    free(walksat->due_to_second_backtrack);
    walksat->due_to_second_backtrack = NULL;

    free(walksat->wr);
    walksat->wr = NULL;

    free(walksat->qsscore[0]);
    walksat->qsscore[0] = NULL;

    free(walksat->qsscore[1]);
    walksat->qsscore[1] = NULL;

    free(walksat->qsscore);
    walksat->qsscore = NULL;

    free(walksat->clause_to_variable_activity);

    free(walksat->temp_list);

    free(walksat->vc);

    free(walksat->vc_v);

    free(walksat->record_priority);
...
void build_walksat_clause(QDPLL* qdpll, WalkSAT* walksat, int use_sat_unit)
{
    int     cur_lit;
    int     i, j;
    int		v, c = 0;//var, clause
    Constraint* _c;

    walksat->num_vars = qdpll->pcnf.size_vars - 1;//变量个数
    walksat->num_clauses = qdpll->pcnf.size_clause;//子句个数
    walksat->unitclause_queue_end_pointer = 0;
    walksat->simplify = 0;

    //printf("SAT_num_vars=%d  SAT_num_clause=%d\n", walksat->num_vars, walksat->num_clauses);

    walksat->ratio = (double)(walksat->num_clauses) / walksat->num_vars;

    if (walksat->num_vars >= SAT_MAX_VARS || walksat->num_clauses >= SAT_MAX_CLAUSES)
    {
        printf("the size of instance exceeds out limitation, please enlarge MAX_VARS and (or) MAX_CLAUSES.\n");
        exit(-1);
    }

    for (c = 0; c < walksat->num_clauses; c++)
    {
        walksat->clause_lit_count[c] = 0;
        walksat->clause_delete[c] = 0;
        walksat->clause_var_count[c] = 0;
    }
    for (v = 1; v <= walksat->num_vars; ++v)
    {
        walksat->var_lit_count[v] = 0;
        walksat->fix[v] = 0;
        walksat->var_type[v] = 2;
    }

    walksat->max_clause_len = 0;
    walksat->min_clause_len = walksat->num_vars;
    walksat->formula_len = 0;
    ...
    Var* var;
    int test_num = 5;
    for (_c = qdpll->pcnf.clauses.first, c = 0; _c; _c = _c->link.next, c++)//_c是每一个子句
    {
        //assert(c == _c->id);
        //printf("%d %d\n", c, _c->id);
        //_c->id == c + 1
        //walksat->clause_lit[c] = (SAT_lit1*)qdpll_malloc(mm, (_c->num_lits + 1) * sizeof(SAT_lit1));
        walksat->clause_qbf_to_sat[_c->id] = c;
        walksat->clause_sat_to_qbf[c] = _c->id;

        //walksat->clause_lit[c] = (SAT_lit*)malloc((_c->num_lits + 1) * sizeof(SAT_lit));
        walksat->clause_lit[c] = (SAT_lit*)qdpll_malloc(mm, (_c->num_lits + 1) * sizeof(SAT_lit));
        walksat->clause_lit_count[c] = _c->num_lits;
        //if(c==0)  printf("c = %d: ", c);
        for (p = _c->lits, e = p + _c->num_lits, i = 0; p < e; p++, i++)//p是c中的每一个文字
        {
            walksat->clause_lit[c][i].clause_num = c;
            walksat->clause_lit[c][i].var_num = abs(*p);
            if (*p > 0) walksat->clause_lit[c][i].sense = 1;//代表为正文字
            else walksat->clause_lit[c][i].sense = 0;

            walksat->var_lit_count[walksat->clause_lit[c][i].var_num]++;
            var = LIT2VARPTR(qdpll->pcnf.vars, *p);//p对应的变量
            if (var->scope->type == QDPLL_QTYPE_EXISTS)//存在文字
            {
                //_c->id == c + 1
                //walksat->clause_var_count[c + 1]++;
                walksat->clause_var_count[c]++;
                walksat->var_type[var->id] = 1;
            }
            else {
                walksat->var_type[var->id] = 0;
            }
        }
        //printf("***\n");
        walksat->clause_lit[c][i].var_num = 0;
        walksat->clause_lit[c][i].clause_num = -1;
        //if(c==1||c==0)  printf("c=%d  i=%d\n",c,i);
        //walksat->clause_lit[c][i].var_num = -1;


        //unit clause
        
        if (walksat->clause_lit_count[c] == 1 && use_sat_unit)
        {
            if (walksat->clause_delete[c] == 1)  continue;
            walksat->unitclause_queue[walksat->unitclause_queue_end_pointer++] = walksat->clause_lit[c][0];
            walksat->clause_delete[c] = 1;
            walksat->cur_soln[walksat->clause_lit[c][0].var_num] = walksat->clause_lit[c][0].sense;
            //fix_num++;
            //if(walksat->clause_lit[c][0].var_num == 3)
              //printf("%d %d  %d  %d*****************\n", walksat->clause_lit[c][0].var_num, walksat->clause_lit[c][0].sense, walksat->cur_soln[walksat->clause_lit[c][0].var_num], _c->num_lits);
        }
        //printf("***111\n");
        
        if (walksat->clause_lit_count[c] > walksat->max_clause_len)
            walksat->max_clause_len = walksat->clause_lit_count[c];
        else if (walksat->clause_lit_count[c] < walksat->min_clause_len)
            walksat->min_clause_len = walksat->clause_lit_count[c];

        walksat->formula_len += walksat->clause_lit_count[c];
    }
    //exit(0);

    //printf("---\n");

    walksat->avg_clause_len = (double)walksat->formula_len / walksat->num_clauses;

    if (walksat->unitclause_queue_end_pointer > 0)
    {
        walksat->simplify = 1;
        for (c = 0; c < walksat->num_clauses; c++)
        {
            walksat->org_clause_lit_count[c] = walksat->clause_lit_count[c];
            walksat->org_clause_lit[c] = (SAT_lit*)qdpll_malloc(mm, (walksat->clause_lit_count[c] + 1) * sizeof(SAT_lit));
            //walksat->org_clause_lit[c] = (SAT_lit*)malloc((walksat->clause_lit_count[c] + 1) * sizeof(SAT_lit));
            for (i = 0; i < walksat->org_clause_lit_count[c]; ++i)
            {
                walksat->org_clause_lit[c][i] = walksat->clause_lit[c][i];
            }
        }
    }
    //test_end();
    ...
    for (v = 1; v <= walksat->num_vars; ++v)
    {
        //walksat->var_lit[v] = (SAT_lit1*)qdpll_malloc(mm, (walksat->var_lit_count[v] + 1) * sizeof(SAT_lit1));
        walksat->var_lit[v] = (SAT_lit*)malloc((walksat->var_lit_count[v] + 1) * sizeof(SAT_lit));
        walksat->var_lit_count[v] = 0;	//reset to 0, for build up the array
    }
    //printf("---------\n");
    //scan all clauses to build up var literal arrays
    for (c = 0; c < walksat->num_clauses; ++c)
    {
        for (i = 0; i < walksat->clause_lit_count[c]; ++i)
        {
            v = walksat->clause_lit[c][i].var_num;
            walksat->var_lit[v][walksat->var_lit_count[v]] = walksat->clause_lit[c][i];
            walksat->var_lit_count[v]++;
        }
    }
    //printf("------------\n");
    for (v = 1; v <= walksat->num_vars; ++v) //set boundary
        walksat->var_lit[v][walksat->var_lit_count[v]].clause_num = -1;

    //printf("fix_num = %d\n", fix_num);
    //printf("*i = %d    walksat->clause_lit_count[i] = %d\n", 1, walksat->clause_lit_count[1]);
}

void walksat_unit_propagation(WalkSAT* walksat)
{
    struct SAT_lit uc_lit;
    int uc_clause;
    int uc_var;
    int uc_sense;
    ...
    int c, v;
    int i, j;
    struct SAT_lit cur, cur_c;


    //while (unitclause_queue_beg_pointer < unitclause_queue_end_pointer) 
    for (walksat->unitclause_queue_beg_pointer = 0; walksat->unitclause_queue_beg_pointer < walksat->unitclause_queue_end_pointer; walksat->unitclause_queue_beg_pointer++)
    {
        uc_lit = walksat->unitclause_queue[walksat->unitclause_queue_beg_pointer];

        uc_var = uc_lit.var_num;
        uc_sense = uc_lit.sense;

        if (walksat->fix[uc_var] == 1)
        {
            if (uc_sense != walksat->cur_soln[uc_var])
            {
                //printf("c wants to fix a variable twice, forbid.\n");
                walksat->satisfy_flag = -1;
                return;
            }
            continue;
        }

        walksat->cur_soln[uc_var] = uc_sense;//fix the variable in unit clause
        walksat->fix[uc_var] = 1;

        for (i = 0; i < walksat->var_lit_count[uc_var]; ++i)
        {
            cur = walksat->var_lit[uc_var][i];
            c = cur.clause_num;
            ...
            {
                if (walksat->clause_lit_count[c] == 2)
                {
                    if (walksat->clause_lit[c][0].var_num == uc_var)
                    {
                        walksat->unitclause_queue[walksat->unitclause_queue_end_pointer++] = walksat->clause_lit[c][1];
                    }
                    else
                    {
                        walksat->unitclause_queue[walksat->unitclause_queue_end_pointer++] = walksat->clause_lit[c][0];
                ...
                {
                    for (j = 0; j < walksat->clause_lit_count[c]; ++j)
                    {
                        if (walksat->clause_lit[c][j].var_num == uc_var)
                        {
                            walksat->clause_lit[c][j] = walksat->clause_lit[c][walksat->clause_lit_count[c] - 1];

                            walksat->clause_lit_count[c]--;
...
void walksat_preprocess(WalkSAT* walksat)
{
    int c, v, i;
    int delete_clause_count = 0;
    int fix_var_count = 0;

    walksat_unit_propagation(walksat);
    //printf("clause_lit_count[544]=%d  walksat->num_clauses=%d\n", walksat->clause_lit_count[544], walksat->num_clauses);
    //printf("walksat->unitclause_queue_end_pointer = %d\n", walksat->unitclause_queue_end_pointer);
    //exit(0);
    if (walksat->satisfy_flag == -1)  return ;
    ...
    for (c = 0; c < walksat->num_clauses; ++c)
    {
        ...
        walksat->clause_lit[c][i].var_num = 0; //new clause boundary
        walksat->clause_lit[c][i].clause_num = -1;

        //about clause length
        formula_len += walksat->clause_lit_count[c];
    ...
    walksat->avg_clause_len = (double)formula_len / walksat->num_clauses;

    for (v = 1; v <= walksat->num_vars; ++v)
    {
        /*
        printf("v = %d: walksat->var_lit_count[v] = %d  : (", v,walksat->var_lit_count[v]);
        for (int x = 0; x < walksat->var_lit_count[v]; x++)
        {
            printf("%3d ", walksat->var_lit[v][x].clause_num);
        }
        printf(")\n");*/
        if (walksat->fix[v] == 1)
        {
        ...
        walksat->var_lit[v][walksat->var_lit_count[v]].clause_num = -1;//new var_lit boundary
    }
    //exit(0);

    //printf("c unit propagation fixes %d variables, and delets %d clauses\n", fix_var_count, delete_clause_count);
}

void walksat_build_neighbor_relation(QDPLL* qdpll, WalkSAT* walksat)
{
    QDPLLMemMan* mm = qdpll->mm;
    int* neighbor_flag = (int*)qdpll_malloc(mm, (walksat->num_vars + 1) * sizeof(int));
    //int* neighbor_flag = (int*)malloc((walksat->num_vars + 1) * sizeof(int));
    int		i, j, count;
    int 	v, c;

    for (i = 1; i <= walksat->num_vars; ++i)
        neighbor_flag[i] = 0;

    for (v = 1; v <= walksat->num_vars; ++v)
    {
        walksat->var_neighbor_count[v] = 0;

        if (walksat->fix[v] == 1) continue;

        neighbor_flag[v] = 1;
        for (i = 0; i < walksat->var_lit_count[v]; ++i)
        {
            c = walksat->var_lit[v][i].clause_num;
            if (walksat->clause_delete[c] == 1) continue;

            for (j = 0; j < walksat->clause_lit_count[c]; ++j)
            {
                if (neighbor_flag[walksat->clause_lit[c][j].var_num] == 0)
                {
                    walksat->var_neighbor_count[v]++;
                    neighbor_flag[walksat->clause_lit[c][j].var_num] = 1;
                }
            }
        }

        neighbor_flag[v] = 0;
        walksat->var_neighbor[v] = (int*)qdpll_malloc(mm, (walksat->var_neighbor_count[v] + 1) * sizeof(int));
        //walksat->var_neighbor[v] = (int*)malloc((walksat->var_neighbor_count[v] + 1) * sizeof(int));
        ...
        for (i = 0; i < walksat->var_lit_count[v]; ++i)
        {
            ...
            for (j = 0; j < walksat->clause_lit_count[c]; ++j)
            {
                if (neighbor_flag[walksat->clause_lit[c][j].var_num] == 1)
                {
                    walksat->var_neighbor[v][count] = walksat->clause_lit[c][j].var_num;
                    count++;
                    neighbor_flag[walksat->clause_lit[c][j].var_num] = 0;
                }
            }
        }
        walksat->var_neighbor[v][count] = 0;
    }
    /*
    for (v = 1; v <= walksat->num_vars; ++v)
    {
        printf("v=%d(%d): ", v, walksat->var_neighbor_count[v]);
        for (i = 0; i < walksat->var_neighbor_count[v]; i++)
        {
            printf("%d ", walksat->var_neighbor[v][i]);
        }
        printf("\n");
    }*/

    //qdpll_free(mm, neighbor_flag, (walksat->num_vars + 1) * sizeof(int));
    //neighbor_flag = NULL;
    free(neighbor_flag);
    //neighbor_flag = NULL;
}

void walksat_set_parameter(WalkSAT* walksat, int max_filps)
{
    walksat->max_flips = max_filps;
    walksat->p_scale = 0.3;
    walksat->q_scale = 0.7;
    walksat->threshold = 50;
    walksat->ave_weight = 1;
    walksat->delta_total_weight = 0;
    walksat->scale_ave = (walksat->threshold + 1) * walksat->q_scale;
    walksat->aspiration_active = 1;
}

void first_walksat_assignment(WalkSAT* walksat)
{
    int fix_num = 0;
    for (int v = 1; v <= walksat->num_vars; v++) {
        //walksat->clause_delete[v] = 0;
        //walksat->qsscore[0][v] = walksat->qsscore[1][v] = walksat->unit_qsscore;
        walksat->time_stamp[v] = 0;
        walksat->freq[v] = 0;
        if (walksat->fix[v] == 0) {

            if (walksat->dpll_fix[v] == 0)
            {
                if (rand() % 2 == 1) walksat->cur_soln[v] = 1;
                else walksat->cur_soln[v] = 0;
            }
            assert(walksat->cur_soln[v] == 1 || walksat->cur_soln[v] == 0);
        }
        //else  fix_num++;
        //else walksat->cur_soln[v] = walksat->var_lit.
    }
}

inline void walksat_unsat(WalkSAT* walksat, int clause)
{
    //if (clause == 523)  printf("523 unsat\n");
    
    walksat->clause_unsat_stamp[clause] = walksat->step;//标记进入unsat栈时的时间戳
    walksat->index_in_unsat_stack[clause] = walksat->unsat_stack_fill_pointer;
    walksat_push(clause, walksat->unsat_stack);

    //update appreance count of each var in unsat clause and update stack of vars in unsat clauses
    int v;
    for (struct SAT_lit* p = walksat->clause_lit[clause]; (v = p->var_num) != 0; p++)
    {
        walksat->unsat_app_count[v]++;
        if (walksat->unsat_app_count[v] == 1)
        {
            walksat->index_in_unsatvar_stack[v] = walksat->unsatvar_stack_fill_pointer;
            walksat_push(v, walksat->unsatvar_stack);
        }
    }
    //printf("unsat  clause=%d    walksat->unsat_stack_fill_pointer=%d\n", clause, walksat->unsat_stack_fill_pointer);
}

inline void walksat_sat(WalkSAT* walksat, int clause)
{
    //if (clause == 523)  printf("523 sat\n");
    
    walksat->clause_unsat_count[clause] += walksat->step - walksat->clause_unsat_stamp[clause];//clause从unsat变为sat
    walksat->clause_unsat_stamp[clause] = -1;//重置

    int index, last_unsat_clause;

    //since the clause is satisfied, its position can be reused to store the last_unsat_clause
    last_unsat_clause = walksat_pop(walksat->unsat_stack);
    index = walksat->index_in_unsat_stack[clause];
    walksat->unsat_stack[index] = last_unsat_clause;
    walksat->index_in_unsat_stack[last_unsat_clause] = index;
    walksat->index_in_unsat_stack[clause] = -1;
    ...
    {
        walksat->unsat_app_count[v]--;
        if (walksat->unsat_app_count[v] == 0)
        {
            last_unsat_var = walksat_pop(walksat->unsatvar_stack);
            index = walksat->index_in_unsatvar_stack[v];
            walksat->unsatvar_stack[index] = last_unsat_var;
            walksat->index_in_unsatvar_stack[last_unsat_var] = index;
        }
    }
    //printf("sat  clause=%d    walksat->unsat_stack_fill_pointer=%d\n", clause, walksat->unsat_stack_fill_pointer);
}

void swap(WalkSAT* walksat, int low, int high) //交换两个数的值
{
    if (low == high)
        return;
    int t = walksat->clause_unsat_count_sort[low].unsat_count;
    int t1 = walksat->clause_unsat_count_sort[low].clause_id;
    walksat->clause_unsat_count_sort[low].unsat_count = walksat->clause_unsat_count_sort[high].unsat_count;
    walksat->clause_unsat_count_sort[low].clause_id = walksat->clause_unsat_count_sort[high].clause_id;
    walksat->clause_unsat_count_sort[high].unsat_count = t;
    walksat->clause_unsat_count_sort[high].clause_id = t1;
}

void adjust(WalkSAT* walksat, int index, int len)
{
    int leftIndex = 2 * index + 1;
    int rightIndex = 2 * index + 2;
    int bigIndex = index;

    if (leftIndex < len && walksat->clause_unsat_count_sort[bigIndex].unsat_count < walksat->clause_unsat_count_sort[leftIndex].unsat_count) {
        bigIndex = leftIndex;
    }

    if (rightIndex < len && walksat->clause_unsat_count_sort[bigIndex].unsat_count < walksat->clause_unsat_count_sort[rightIndex].unsat_count) {
        bigIndex = rightIndex;
    }

    if (bigIndex != index) {
        swap(walksat, index, bigIndex);
        adjust(walksat, bigIndex, len);
    }
}

void heapsort(WalkSAT* walksat, int low, int high)
{
    int len = high - low + 1;
    for (int i = (len / 2) - 1; i >= 0; i--) {
        adjust(walksat, i, len);
    }

    for (int i = 0; i < len; i++) {
        swap(walksat, 0, len - 1 - i);
        adjust(walksat, 0, len - 1 - i);
    }
}

void Mark_desired_liter(WalkSAT* walksat)
{
    for (int i = 0, c; i < walksat->unsat_stack_fill_pointer; i++)
    {
        c = walksat->unsat_stack[i];
        walksat->clause_unsat_count[c] += walksat->step - walksat->clause_unsat_stamp[c];
    }

    walksat->num_clause_unsat_count_sort = 0;

    printf("111\n");

    /*
    for (int c = 0; c < walksat->num_clauses; c++)
    {
        if (walksat->clause_unsat_count[c] > 1)
        {
            walksat->clause_unsat_count_sort[walksat->num_clause_unsat_count_sort].clause_id = c;
            walksat->clause_unsat_count_sort[walksat->num_clause_unsat_count_sort].unsat_count = walksat->clause_unsat_count[c];
            walksat->num_clause_unsat_count_sort++;
        }
    }*/

    for (int c = 0; c < walksat->num_clauses; c++)
    {
        if (walksat->clause_weight[c] >= 2)
        {
            walksat->clause_unsat_count_sort[walksat->num_clause_unsat_count_sort].clause_id = c;
            walksat->clause_unsat_count_sort[walksat->num_clause_unsat_count_sort].unsat_count = walksat->clause_weight[c];
            walksat->num_clause_unsat_count_sort++;
        }
    }
    ...
    heapsort(walksat, 0, walksat->num_clause_unsat_count_sort - 1);
    /*
    for (int i = 0, c; i < walksat->num_clause_unsat_count_sort; i++)
    {
        c = walksat->clause_unsat_count_sort[i].clause_id;
        printf("*c=%d  unsat_num=%d\n", c, walksat->clause_unsat_count[c]);
    }
    printf("\n\n");*/
    //exit(0);

    printf("walksat->num_clause_unsat_count_sort = %d\n", walksat->num_clause_unsat_count_sort);

    int pos = walksat->num_clause_unsat_count_sort * (1.0 - walksat->hope_proportion);
    int num = 0;
    printf("pos=%d  walksat->num_clause_unsat_count_sort=%d  c_num=%d  v_num=%d\n", pos, walksat->num_clause_unsat_count_sort,walksat->num_clauses,walksat->num_vars);

    for (int i = pos, c, v, j; i < walksat->num_clause_unsat_count_sort; i++)
    {
        //printf("i=%d ", i);
        c = walksat->clause_unsat_count_sort[i].clause_id;
        walksat->had_vis[c] = 1;//这些子句的dpll_fix文字都变成is_hope了，所以不用看了
        //printf("c=%d: ", c);
        for (j = 0; j < walksat->clause_lit_count[c]; j++)
        {
            v = walksat->clause_lit[c][j].var_num;
            if (walksat->dpll_fix[v] == 1)
            {
                //if (!walksat->is_hope[v])
                //{
                    //num++;
                    //printf("%d ", v);
                //}
    ...
    printf("\nnum = %d\n", num);
    if (walksat->use_second_unknown_backtrack_strategy)//采用第二种unknown backtrack选择有希望顶点策略
    {
        struct SAT_lit* clause_c, * q;
        int c, v, v1, i;

        //printf("111\n");
        for (v = 1; v <= walksat->num_vars; v++) {
            if (walksat->dpll_fix[v] == 1 && walksat->is_hope[v] == 1)
            {
                //只检查dpll固定的，且是有希望的顶点，所在的子句
                for (q = walksat->var_lit[v]; (c = q->clause_num) >= 0; q++)
                {
                    if (walksat->had_vis[c] || walksat->sat_count[c] != 1)  continue;
                    //printf("111.1\n");
                    walksat->had_vis[c] = 1;//标记检查过这个子句

                    if (walksat->cur_soln[v] == q->sense)//确定是v满足该子句,而且只有v一个变量满足
                    {
                        for (i = 0; i < walksat->clause_lit_count[c]; ++i)
                        {
                            v1 = walksat->clause_lit[c][i].var_num;
                            if (walksat->dpll_fix[v1] == 1 && walksat->is_hope[v1] == 0)
                            {
                                num++;
                                walksat->is_hope[v1] = 1;
                                walksat->due_to_second_backtrack[v1] = 1;
                            }
                        }
                    }
                }
    ...
    while(1)
    {
        ...
        for (j = 0; j < walksat->clause_lit_count[c]; j++)
        {
            ...
            printf("%d(%d) ", v,i);
            if (v > walksat->num_vars || v < 1)
            {
                printf("\nv > walksat->num_vars || v < 1    v=%d  i=%d  j=%d\n", v,i,j);
                printf("delete=%d  walksat->clause_lit_count[c]=%d\n", walksat->clause_delete[c], walksat->clause_lit_count[c]);
                exit(0);
            }
        }
        printf("*\n");
        ...
        i++;
        printf("i++ ");
        if (i >= walksat->num_clause_unsat_count_sort)  break;
        printf("next ");
    }
    printf("\nnum = %d\n", num);*/
    ...
    for (int c = 0; c < walksat->num_clauses; c++)
    {
        if (walksat->clause_unsat_count[c] > 0)
        {
            printf("c=%d  walksat->clause_unsat_count[c]=%d\n", c, walksat->clause_unsat_count[c]);
        }
    }
    printf("walksat->unsat_stack_fill_pointer = %d\n", walksat->unsat_stack_fill_pointer);
    for (int i = 0, c; i < walksat->unsat_stack_fill_pointer; i++)
    {
        c = walksat->unsat_stack[i];
        printf("*c=%d  walksat->clause_unsat_count[c]=%d\n", c, walksat->clause_unsat_count[c]);
...
void walksat_smooth_clause_weights(WalkSAT* walksat)
{
    int i, j, c, v;
    ...
    for (v = 1; v <= walksat->num_vars; ++v)
    {
        if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
        walksat->score[v] = 0;
    }
    ...
    for (c = 0; c < walksat->num_clauses; ++c)
    {
        walksat->clause_weight[c] = walksat->clause_weight[c] * walksat->p_scale + walksat->scale_ave;
        if (walksat->clause_weight[c] < 1) walksat->clause_weight[c] = 1;

        new_total_weight += walksat->clause_weight[c];

        //update score of variables in this clause 
        if (walksat->sat_count[c] == 0)
        {
            for (j = 0; j < walksat->clause_lit_count[c]; ++j)
            {
                v = walksat->clause_lit[c][j].var_num;
                if (v <= 0 || v > walksat->num_vars)  continue;
                if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
                walksat->score[v] += walksat->clause_weight[c];
            }
        }
        else  if (walksat->sat_count[c] == 1)
        {
            v = walksat->sat_var[c];
            if (v <= 0 || v > walksat->num_vars)  continue;
            if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
        ...
        if (walksat->sat_count[c] == 0)
        {
            ...
            {
                if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
                walksat->score[walksat->clause_lit[c][j].var_num] += walksat->clause_weight[c];
            }
        }
        else  if (walksat->sat_count[c] == 1)
        {
            if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
            walksat->score[walksat->sat_var[c]] -= walksat->clause_weight[c];
        }*/
    }

    walksat->ave_weight = new_total_weight / walksat->num_clauses;
    walksat->sum_weight = new_total_weight % walksat->num_clauses;
    walksat->avg_weight = walksat->ave_weight;
}

void walksat_update_clause_weights(WalkSAT* walksat)
{
    int i, v;

    for (i = 0; i < walksat->unsat_stack_fill_pointer; ++i)
        walksat->clause_weight[walksat->unsat_stack[i]]++;
    walksat->sum_weight += walksat->unsat_stack_fill_pointer;
    walksat->avg_weight += walksat->sum_weight / walksat->num_clauses;
    walksat->sum_weight %= walksat->num_clauses;

    for (i = 0; i < walksat->unsatvar_stack_fill_pointer; ++i)
    {
        v = walksat->unsatvar_stack[i];
        if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
        walksat->score[v] += walksat->unsat_app_count[v];
        if (walksat->score[v] > 0 && walksat->conf_change[v] == 1 && walksat->already_in_goodvar_stack[v] == 0 && walksat->dpll_fix[v] == 0)
        //if (walksat->score[v] > 0 && walksat->already_in_goodvar_stack[v] == 0 && walksat->dpll_fix[v] == 0)//不使用CC策略
        {
            walksat_push(v, walksat->goodvar_stack);
            walksat->already_in_goodvar_stack[v] = 1;
        }
    }

    walksat->delta_total_weight += walksat->unsat_stack_fill_pointer;
    if (walksat->delta_total_weight >= walksat->num_clauses)
    {
        walksat->ave_weight += 1;
        walksat->delta_total_weight -= walksat->num_clauses;

        //smooth weights
        if (walksat->ave_weight > walksat->threshold)
            walksat_smooth_clause_weights(walksat);
    }
}

int walksat_pick_var_use_guide_reward(WalkSAT* walksat)
{
    walksat->reward_score = walksat->avg_weight * walksat->reward_ratio;
    if (walksat->reward_score < 1)  walksat->reward_score = 1;
    /*
    int sum = 0;
    for (int c = 0; c < walksat->num_clauses; c++)
    {
        sum += walksat->clause_weight[c];
    }
    if (sum / walksat->num_clauses != walksat->avg_weight)
    {
        printf("sum / walksat->num_clauses != walksat->avg_weight\n");
        exit(0);
    }*/
    //
    ...
    int         i, k, c, v;
    int         best_var = -1;
    struct SAT_lit* clause_c;
    int bv, bbest;
    int bv_qsscore, bbest_qsscore;
    int score_v, score_best;

    int p = 90;//50%的概率选择打分最高的变量，50%的概率随机从未满足子句中选择一个变量
    /**Greedy Mode**/
    /*CCD (configuration changed decreasing) mode, the level with configuation chekcing*/
    //printf("before good\n");
    //if (rand() % 100 < p)
    {
        if (walksat->goodvar_stack_fill_pointer > 0)//50%的概率
        {
            /*
            for (i = 0; i < walksat->goodvar_stack_fill_pointer; ++i)
            {
                if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
                v = walksat->goodvar_stack[i];
                if (walksat->dpll_fix[v])
                {
                    printf("walksat->dpll_fix[v]\n");
                    exit(0);
                }
            }*/
            best_var = -1;
            for (i = 0; i < walksat->goodvar_stack_fill_pointer; ++i)
            {
                ...
                //如果v的赋值为0（walksat->cur_soln[v]=0），那么此时可能需要v从0到1，所以看的是qsscore[1][v]
                //如果v的赋值为1（walksat->cur_soln[v]=1），那么此时可能需要v从1到0，所以看的是qsscore[0][v]
                //因此，看的是qsscore[ 1 - walksat->cur_soln[v] ][v]
                bv = 1 - walksat->cur_soln[v];
                bbest = 1 - walksat->cur_soln[v];
                score_v = 0;
                score_best = 0;
                if (walksat->qsscore[bv][v] > walksat->qsscore[bbest][best_var])
                {
                    score_v = walksat->reward_score;
                    walksat->big++;
                }
                else if (walksat->qsscore[bv][v] < walksat->qsscore[bbest][best_var])
                {
                    score_best = walksat->reward_score;
                    walksat->small++;
                }
                else {
                    walksat->equal++;
                }
                //if(walksat->reward_score > 0) printf("%d  %d  %d\n", walksat->score[v], walksat->score[best_var], walksat->reward_score);
                if (walksat->score[v] + score_v > walksat->score[best_var] + score_best) best_var = v;
                else if (walksat->score[v] + score_v == walksat->score[best_var] + score_best && walksat->time_stamp[v] < walksat->time_stamp[best_var]) best_var = v;
            }
            //printf("111\n");
            return best_var;
        }
        ...
        //aspiration
        //暂时walkqsat不使用
        if (walksat->aspiration_active)
        {
            best_var = 0;
            for (i = 0; i < walksat->unsatvar_stack_fill_pointer; ++i)
            {
                if (walksat->dpll_fix[walksat->unsatvar_stack[i]] || walksat->fix[walksat->unsatvar_stack[i]])  continue;
                if (walksat->score[walksat->unsatvar_stack[i]] > walksat->ave_weight)
                {
                    best_var = walksat->unsatvar_stack[i];
                    break;
                }
            }

            for (++i; i < walksat->unsatvar_stack_fill_pointer; ++i)
            {
                v = walksat->unsatvar_stack[i];
                if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
                if (best_var == 0)
    ...
    //printf("before update w\n");
    //暂时不用
    walksat_update_clause_weights(walksat);

    /*focused random walk*/
    //printf("before unsat  walksat->unsat_stack_fill_pointer=%d\n", walksat->unsat_stack_fill_pointer);
    c = walksat->unsat_stack[rand() % walksat->unsat_stack_fill_pointer];
    clause_c = walksat->clause_lit[c];
    //best_var = clause_c[0].var_num;//取出变量
    best_var = -1;
    //printf("before unsat 111\n");
    for (k = 0; k < walksat->clause_lit_count[c]; ++k)
    {
        ...
        if (best_var == -1) best_var = v;
        else if (walksat->unsat_app_count[v] > walksat->unsat_app_count[best_var]) best_var = v;
        //else if(unsat_app_count[v]==unsat_app_count[best_var] && time_stamp[v]<time_stamp[best_var]) best_var = v;
        else if (walksat->unsat_app_count[v] == walksat->unsat_app_count[best_var])
        {
            //如果v的赋值为0（walksat->cur_soln[v]=0），那么此时可能需要v从0到1，所以看的是qsscore[1][v]
            //如果v的赋值为1（walksat->cur_soln[v]=1），那么此时可能需要v从1到0，所以看的是qsscore[0][v]
            //因此，看的是qsscore[ 1 - walksat->cur_soln[v] ][v]
...
static void backtrack_sat_bcp(WalkSAT* walksat)
{
    ...
    for (c = 0; c < walksat->num_clauses; c++)
    {
        walksat->clause_delete[c] = 0;//2.恢复所有标志为delete的子句
        walksat->clause_lit_count[c] = walksat->org_clause_lit_count[c];
        for (i = 0; i <= walksat->org_clause_lit_count[c]; ++i)
        {
            walksat->clause_lit[c][i] = walksat->org_clause_lit[c][i];
        }
    }
...
void walksat_flip(WalkSAT* walksat, int flipvar, int nnum)
{
    walksat->cur_soln[flipvar] = 1 - walksat->cur_soln[flipvar];
    ...
    int org_flipvar_score = walksat->score[flipvar];

    //update related clauses and neighbor vars
    for (struct SAT_lit* q = walksat->var_lit[flipvar]; (c = q->clause_num) >= 0; q++)
    {
        if (walksat->clause_delete[c] > 0)  continue;//防止将阻塞的子句当做为满足子句
        clause_c = walksat->clause_lit[c];
        if (walksat->cur_soln[flipvar] == q->sense)
        {
            ++walksat->sat_count[c];

            if (walksat->sat_count[c] == 2) //sat_count from 1 to 2
            {
                walksat->score[walksat->sat_var[c]] += walksat->clause_weight[c];
            }
            else if (walksat->sat_count[c] == 1) // sat_count from 0 to 1
            {
                //printf("walksat->sat_count[c] == 1\n");
                walksat->sat_var[c] = flipvar;//record the only true lit's var
                for (struct SAT_lit* p = clause_c; (v = p->var_num) != 0; p++) walksat->score[v] -= walksat->clause_weight[c];

                walksat_sat(walksat, c);
            }
        ...
        {
            --walksat->sat_count[c];
            if (walksat->sat_count[c] == 1) //sat_count from 2 to 1
            {
                for (struct SAT_lit* p = clause_c; (v = p->var_num) != 0; p++)
                {
                    if (p->sense == walksat->cur_soln[v])
                    {
                        walksat->score[v] -= walksat->clause_weight[c];
                        walksat->sat_var[c] = v;
                        break;
            ...
            else if (walksat->sat_count[c] == 0) //sat_count from 1 to 0
            {
                for (struct SAT_lit* p = clause_c; (v = p->var_num) != 0; p++) walksat->score[v] += walksat->clause_weight[c];
                walksat_unsat(walksat, c);
            }//end else if
    ...
    walksat->score[flipvar] = -org_flipvar_score;
    /*update CCD */
    int index;

    walksat->conf_change[flipvar] = 0;
    //remove the vars no longer goodvar in goodvar stack 
    for (index = walksat->goodvar_stack_fill_pointer - 1; index >= 0; index--)
    {
        v = walksat->goodvar_stack[index];
        if (walksat->score[v] <= 0)
        {
            walksat->goodvar_stack[index] = walksat_pop(walksat->goodvar_stack);
            walksat->already_in_goodvar_stack[v] = 0;
        }
    }

    //update all flipvar's neighbor's conf_change to be 1, add goodvar
    int* p;
    for (p = walksat->var_neighbor[flipvar]; (v = *p) != 0; p++)
    {
        walksat->conf_change[v] = 1;
        if (walksat->score[v] > 0 && walksat->already_in_goodvar_stack[v] == 0 && walksat->dpll_fix[v] == 0)
        {
            walksat_push(v, walksat->goodvar_stack);
            walksat->already_in_goodvar_stack[v] = 1;
        }
...
int walksat_verify_sol(QDPLL* qdpll, WalkSAT* walksat)
{
    int c, j;
    int flag;

    if (walksat->simplify == 0)
    {
        ...
        for (c = 0; c < walksat->num_clauses; ++c)
        {
            ...
            flag = 0;
            for (j = 0; j < walksat->clause_lit_count[c]; ++j)
                if (walksat->cur_soln[walksat->clause_lit[c][j].var_num] == walksat->clause_lit[c][j].sense) { flag = 1; break; }

            if (flag == 0) {//output the clause unsatisfied by the solution

                printf("c clause %d is not satisfied\n", c);

                if (walksat->clause_delete[c] > 0)  printf("c this clause is deleted by UP.\n");

                printf("c ");
                for (j = 0; j < walksat->clause_lit_count[c]; ++j)
                {
                    if (walksat->clause_lit[c][j].sense == 0)  printf("-");
                    printf("%d ", walksat->clause_lit[c][j].var_num);
                }
                printf("\n");

                for (j = 0; j < walksat->clause_lit_count[c]; ++j)
                    printf("%d ", walksat->cur_soln[walksat->clause_lit[c][j].var_num]);
                printf("\n");

                for (j = 0; j < walksat->clause_lit_count[c]; ++j)
                    printf("%d ", walksat->dpll_fix[walksat->clause_lit[c][j].var_num]);
                printf("\n");

                for (j = 0; j < walksat->clause_lit_count[c]; ++j)
                    printf("%d ", walksat->fix[walksat->clause_lit[c][j].var_num]);
                printf("\n");

                printf("walksat->sat_count[c] = %d\n", walksat->sat_count[c]);
                ...
                if (1)
                {
                        ...
                        se = qdpll->qbcp_qbce_blocked_clauses.top; sp < se; sp++)
                    {
                        ...
                        {
                            Constraint* _c = *cp;
                            assert(_c->qbcp_qbce_blocked);
                            printf("c=%d    _c->id = %d     walksat->clause_qbf_to_sat[_c->id]=%d    walksat->clause_delete[%d]=%d\n", 
                                c, _c->id, walksat->clause_qbf_to_sat[_c->id], walksat->clause_qbf_to_sat[_c->id],
                                walksat->clause_delete[walksat->clause_qbf_to_sat[_c->id]]);
                            if (walksat->clause_qbf_to_sat[_c->id] == c)
                            {
                                print_constraint(qdpll, _c);
                                printf("_c->id == c + 1\n");
                                exit(0);
                            }
                        }
                    ...
                    printf("walksat->clause_sat_to_qbf[%d] = %d\n", c, walksat->clause_sat_to_qbf[c]);

                    Constraint* _c;
                    for (_c = qdpll->pcnf.clauses.first; _c; _c = _c->link.next)
                    {
                        if (_c->id == walksat->clause_sat_to_qbf[c])
                        {
                            print_constraint(qdpll, _c);
                        }
                    }
                ...
                {
                    LitID lit;
                    QDPLLAssignment lit_value;
                    unsigned int lit_mode;
                    VarID* vp;
                    VarID* ve;
                    Var* v;
                    //printf("assignment_from_dpll_to_walksat: num = %d  $$$$$$$$$$$$$$$$$$$$$$$$$$$$\n", qdpll->assigned_vars_top - qdpll->assigned_vars);
                    for (vp = qdpll->assigned_vars, ve = qdpll->assigned_vars_top; vp < ve; vp++)
                    {
                        // printf("vp=%d\n", *vp);
                        v = VARID2VARPTR(qdpll->pcnf.vars, *vp);
                        //if(v->id == 188 || v->id == 238 || v->id == 239)  printf("v->id=%d  v->assign=%d\n", v->id,v->assignment);
                        assert(QDPLL_VAR_ASSIGNED(v));
                        //printf("vp=%d  222\n", *vp);
                        walksat->dpll_fix[v->id] = 1;
                        walksat->freq[v->id]++;
                        if (QDPLL_VAR_ASSIGNED_TRUE(v))
                        {
                            if (walksat->fix[v->id] == 1 && walksat->cur_soln[v->id] == 0)
                            {
                                printf("walksat->fix[v->id] == 1 && walksat->cur_soln[v->id] == 0  mode=%d\n", v->mode);
                                //printf("%d\n", v->mode);
                                //exit(0);
                            }
                            walksat->cur_soln[v->id] = 1;
                        }
                        else {
                            if (walksat->fix[v->id] == 1 && walksat->cur_soln[v->id] == 1)
                            {
                                printf("walksat->fix[v->id] == 1 && walksat->cur_soln[v->id] == 1  mode=%d\n", v->mode);
                                //exit(0);
                            }
                            walksat->cur_soln[v->id] = 0;
                        }
                    }
                }
    ...
    if (walksat->simplify == 1)
    {
        for (c = 0; c < walksat->num_clauses; ++c)
        {
            if (walksat->clause_delete[c] == 1)  continue;
            flag = 0;
            for (j = 0; j < walksat->org_clause_lit_count[c]; ++j)
                if (walksat->cur_soln[walksat->org_clause_lit[c][j].var_num] == walksat->org_clause_lit[c][j].sense) { flag = 1; break; }

            if (flag == 0) {//output the clause unsatisfied by the solution

                //cout << "" << c << "" << endl;
                printf("org: c clause %d is not satisfied\n", c);
                ...
                for (j = 0; j < walksat->org_clause_lit_count[c]; ++j)
                {
                    if (walksat->org_clause_lit[c][j].sense == 0)  printf("-");
                    printf("%d ", walksat->org_clause_lit[c][j].var_num);
                }
                printf("\n");

                for (j = 0; j < walksat->org_clause_lit_count[c]; ++j)
                    printf("%d(%d) ", walksat->cur_soln[walksat->org_clause_lit[c][j].var_num], walksat->fix[walksat->org_clause_lit[c][j].var_num]);
                printf("\n");
...
int walksat_pick_var(QDPLL * qdpll, WalkSAT* walksat, int flipvar, int nnum)
{
    int         i, k, c, v;
    int         best_var = -1;
    struct SAT_lit* clause_c;
    ...
    if (rand() % 100 < p)
    {
        ...
        {
            //printf("in good\n");
            //printf("in walksat->goodvar_stack_fill_pointer\n");
            /*
            for (i = 0; i < walksat->goodvar_stack_fill_pointer; ++i)
            {
                if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
            ...
            for (i = 0; i < walksat->goodvar_stack_fill_pointer; ++i)
            {
                ...
                if (walksat->score[v] > walksat->score[best_var]) best_var = v;
                else if (walksat->score[v] == walksat->score[best_var] && walksat->time_stamp[v] < walksat->time_stamp[best_var])  best_var = v;
            }
            /*
            if (best_var == -1)
            {
                ...
                {
                    v = walksat->goodvar_stack[i];
                    printf("%d %d %d %d\n", i, v, walksat->dpll_fix[v], walksat->fix[v]);
                }
                exit(0);
            }*/
            if (best_var != -1)
    ...
    {
        v = clause_c[k].var_num;
        //printf("c=%d  v=%d  dpll_fix=%d  fix=%d\n", c, v, walksat->dpll_fix[v], walksat->fix[v]);
        if (walksat->dpll_fix[v] || walksat->fix[v])  continue;
    ...
    if (best_var == -1)
    {
            ...
            se = qdpll->qbcp_qbce_blocked_clauses.top; sp < se; sp++)
        {
            ...
            for (cp = stack.start, ce = stack.top; cp < ce; cp++)
            {
                Constraint* _c = *cp;
                assert(_c->qbcp_qbce_blocked);
                printf("c=%d    _c->id = %d     walksat->clause_qbf_to_sat[_c->id]=%d\n", c, _c->id, walksat->clause_qbf_to_sat[_c->id]);
                if (walksat->clause_qbf_to_sat[_c->id] == c)
                {
                    print_constraint(qdpll, _c);
                    printf("_c->id == c + 1\n");
...
void walksat_local_search(QDPLL * qdpll, WalkSAT* walksat, double _time_limit, double _strat)
{
    int flipvar;
    unsigned int notime = 1 + walksat->max_flips;
    //walksat->step = 0;//在init里面置0了
    
    int nnum = 0, i, j;
    double end;
    while (--notime)
    {
        if (walksat->unsat_stack_fill_pointer == 0)
        {
            return;
        }
        walksat->step++;
        if (walksat->step % 100 == 0)
        {
            double end1 = clock();
            double t = (end1 - _strat) / CLOCKS_PER_SEC;
            if (t > _time_limit)
            {
                return;
            }
        ...
        //printf("before pick\n");
        flipvar = walksat_pick_var(qdpll, walksat, flipvar, nnum);
        if (flipvar == -1)
        {
            printf("flipvar == -1\n");
            break;
        }
        //if (walksat->score[flipvar] < 0)  walksat->record_activity = 1;
        //printf("flipvar = %d    v_num=%d\n", flipvar, walksat->num_vars);
        walksat_flip(walksat, flipvar, nnum);
         if (walksat->goodvar_stack_fill_pointer == 0)  walksat->record_activity = 1;
        //printf("after flip\n");
        walksat->time_stamp[flipvar] = walksat->step;

        //printf("walksat->unsat_stack_fill_pointer = %d\n", walksat->unsat_stack_fill_pointer);

        /*
        if (walksat->cur_soln[flipvar] == 1)
        {
            walksat->freq[flipvar]++;//加入解集的次数加一
        }

        if (walksat->unsat_stack_fill_pointer < walksat->this_try_best_unsat_stack_fill_pointer)
        {
            walksat->this_try_best_unsat_stack_fill_pointer = walksat->unsat_stack_fill_pointer;
            //notime = 1 + walksat->max_flips;
        }*/
        
        if (walksat->record_activity == 1)
        {
            for (i = 0; i < walksat->unsat_stack_fill_pointer; i++)
            {
                j = walksat->unsat_stack[i];
                //walksat->clause_to_variable_activity[j]++;
                //walksat->clause_to_variable_activity[j] = 1;
                walksat->record_priority[j] = 1;
            }
        }
    }
...
void assignment_from_dpll_to_walksat(QDPLL* qdpll, WalkSAT* walksat)
{
    LitID lit;
    QDPLLAssignment lit_value;
    unsigned int lit_mode;
    ...
    if (!qdpll->options.no_qbce_dynamic)//使用了动态子句消除，所以需要在搜索时，不能管阻塞子句
    {
        int c;
        ConstraintPtrStack* sp, * se;
        for (sp = qdpll->qbcp_qbce_blocked_clauses.start,
            se = qdpll->qbcp_qbce_blocked_clauses.top; sp < se; sp++)
        {
            ...
            for (cp = stack.start, ce = stack.top; cp < ce; cp++)
            {
                Constraint* _c = *cp;
                assert(_c->qbcp_qbce_blocked);
                c = walksat->clause_qbf_to_sat[_c->id];

                if (c<0 || c>walksat->num_clauses)
                {
                    printf("_c->id=%d  c=%d  walksat->num_vars=%d    c<0 || c>walksat->num_vars\n", _c->id,c, walksat->num_clauses);
                    exit(0);
                }
...
void walksat_init(QDPLL* qdpll, WalkSAT* walksat)
{
    walksat->satisfy_flag = 0;
    walksat->record_activity = 0;//第一次出现（未满足子句数量）上升时(即翻转打分为负时)，才开始计算活跃度
    walksat->avg_qsscore = 0;
    for (int v = 1; v <= walksat->num_vars; v++) {
        walksat->dpll_fix[v] = 0;
        walksat->is_hope[v] = 0;
        walksat->due_to_second_backtrack[v] = 0;
        //if(qdpll->_polarity[v] != 2)  walksat->cur_soln[v] = (qdpll->_polarity[v] + 1) / 2;//-1 -> 0, 1 -> 1, 将更新的赋值传给LS
        if (qdpll->_polarity_LS[v] != 2)  walksat->cur_soln[v] = (qdpll->_polarity_LS[v] + 1) / 2;//-1 -> 0, 1 -> 1, 将更新的赋值传给LS
        //walksat->record_priority[v] = 0;
        if(qdpll->no_qcdcl_guide_ls == 1)  walksat->cur_soln[v] = rand() % 2;
    }
    for (int c = 0; c <= walksat->num_clauses; c++)
    {
        walksat->record_priority[c] = 0;
    }
    //for (int c = 0; c < walksat->num_clauses; c++)
    {
        //walksat->clause_delete[c] = 0;
    ...
    //先将dpll的赋值给walksat
    //printf("%d  %d***111.1\n", walksat->cur_soln[3], walksat->cur_soln[276]);
    assignment_from_dpll_to_walksat(qdpll, walksat);
    //printf("%d  %d***111.2\n", walksat->cur_soln[3], walksat->cur_soln[276]);
    //sat_bcp_unit(qdpll,walksat);

    if (walksat->qbf_new_init)
    {
        //ls_qbf_init(qdpll, walksat);//新的初始化，一开始的时候让任意文字尽可能少满足子句
        //ls_qbf_init1(qdpll, walksat);//新的初始化，一开始的时候让任意文字尽可能多满足子句
    }
    //printf("^^^\n");
    ...
    walksat->step = 0;//局部搜索时间戳置0

    //Initialize edge weights
    for (c = 0; c < walksat->num_clauses; c++)
    {
        walksat->had_vis[c] = 0;
        walksat->clause_weight[c] = 1;
        walksat->clause_unsat_count[c] = 0;//子句c不满足的次数
        walksat->clause_unsat_stamp[c] = -1;
        walksat->clause_to_variable_activity[c] = 0;
        walksat->index_in_unsat_stack[c] = -1;
    }
    walksat->sum_weight = 0;
    walksat->avg_weight = 1;

    //init unsat_stack
    walksat->unsat_stack_fill_pointer = 0;
    walksat->unsatvar_stack_fill_pointer = 0;

    //init solution
    for (v = 1; v <= walksat->num_vars; v++) {
        if (walksat->fix[v] == 0 && walksat->dpll_fix[v] == 0)
        {
            /*
            if (walksat->dpll_fix[v] == 0)
            {
            ...
            walksat->time_stamp[v] = 0;
            walksat->conf_change[v] = 1;
            walksat->unsat_app_count[v] = 0;
        }
        //else walksat->cur_soln[v] = walksat->var_lit.
    }
    //printf("walksat->num_clauses = %d\n", walksat->num_clauses);

    long long hash_value = 0;
    long long temp, c_num = 0;
    for (c = 0; c < walksat->num_clauses; ++c)
    {
        //printf("c = %d    +++++++++++++++++++++++++++\n", c);
        //if (c == 523)  printf("walksat->clause_delete[523] = %d\n", walksat->clause_delete[c]);
        //if (c == 767)  printf("walksat->clause_delete[767] = %d\n", walksat->clause_delete[c]);
        if (walksat->clause_delete[c] > 0) continue;

        walksat->sat_count[c] = 0;

        for (j = 0; j < walksat->clause_lit_count[c]; ++j)
        {
            v = walksat->clause_lit[c][j].var_num;
            if (walksat->cur_soln[v] == walksat->clause_lit[c][j].sense)
            {
                walksat->sat_count[c]++;
                walksat->sat_var[c] = walksat->clause_lit[c][j].var_num;
            }
        }

        //if (c == 523)  printf("walksat->sat_count[523] = %d\n", walksat->sat_count[c]);
        //if (c == 767)  printf("walksat->sat_count[767] = %d\n", walksat->sat_count[c]);

        if (walksat->sat_count[c] == 0)
        {
            walksat_unsat(walksat, c);
            c_num++;
            temp = (((c + 1) % walksat->size_hash) * (c_num % walksat->size_hash)) % walksat->size_hash;
            hash_value = ( hash_value + temp ) % walksat->size_hash;
        }
    }

    walksat->is_repeat = 0;
    if (walksat->use_hash == 1 && walksat->hash[hash_value] == 1)//重复了，就不用再调用ls
    {
        walksat->is_repeat = 1;
        //printf("hash_value = %d    walksat->hash[hash_value] = %d    c_num = %d\n", hash_value, walksat->hash[hash_value], c_num);
    }
    walksat->hash[hash_value] = 1;//标记已调用过ls
    ...
    double ttt = (clock() - ttt1) / CLOCKS_PER_SEC;
    */
    //qdpll->time_of_hash += ttt;
    ...
    for (v = 1; v <= walksat->num_vars; v++)
    {
        if (walksat->fix[v] == 1 || walksat->dpll_fix[v] == 1)
        {
            walksat->score[v] = -2000000000;
            continue;
        }

        walksat->score[v] = 0;

        lit_count = walksat->var_lit_count[v];

        for (i = 0; i < lit_count; ++i)
        {
            c = walksat->var_lit[v][i].clause_num;
            if (walksat->sat_count[c] == 0) walksat->score[v]++;
            else if (walksat->sat_count[c] == 1 && walksat->var_lit[v][i].sense == walksat->cur_soln[v]) walksat->score[v]--;
        }
    }

    //init goodvars stack
    walksat->goodvar_stack_fill_pointer = 0;
    for (v = 1; v <= walksat->num_vars; v++)
    {
        if (walksat->fix[v] == 1 || walksat->dpll_fix[v] == 1)  continue;
        if (walksat->score[v] > 0 && walksat->conf_change[v]==1)
        //if (walksat->score[v] > 0)//不使用CC策略
        {
            walksat->already_in_goodvar_stack[v] = 1;
            walksat_push(v, walksat->goodvar_stack);
        }
        else walksat->already_in_goodvar_stack[v] = 0;
    }

    //setting for the virtual var 0
    walksat->time_stamp[0] = 0;
    //pscore[0]=0;

    walksat->this_try_best_unsat_stack_fill_pointer = walksat->unsat_stack_fill_pointer;

    if (qdpll->use_variable_activity == 1)
    {
        //printf("walksat->num_vc = %d\n", walksat->num_vc);
        for (int i = 0, v; i < walksat->num_vc; i++)
        {
            v = walksat->vc[i];
            walksat->dpll_fix[v] = 1;
        }
    }
    ...
    {
        //if (walksat->fix[v] == 1 || walksat->dpll_fix[v] == 1)  continue;
        printf("score[%d]=%d  fix=%d\n", v, walksat->score[v], walksat->fix[v]+walksat->dpll_fix[v]);
    }*/
    //test_end();
}

void walksat_back_block_clause(QDPLL* qdpll, WalkSAT* walksat)
{
    if (!qdpll->options.no_qbce_dynamic)//使用了动态子句消除，所以之前需要在搜索时，不能管阻塞子句，现在需要把它们恢复回来
    {
        int c;
        ConstraintPtrStack* sp,* se;
        for (sp = qdpll->qbcp_qbce_blocked_clauses.start,
            se = qdpll->qbcp_qbce_blocked_clauses.top; sp < se; sp++)
        {
            ...
            for (cp = stack.start, ce = stack.top; cp < ce; cp++)
            {
                ...
                c = walksat->clause_qbf_to_sat[_c->id];
                walksat->clause_delete[c]--;
                if (walksat->clause_delete[c] < 0)
                {
                    printf("walksat->clause_delete[c] < 0\n");
                    exit(0);
                }
            }
        }
...
void check_dp(QDPLL * qdpll, WalkSAT * walksat)
{
    int v, ass;
    for (int i = 0; i < num_dp; i++)
    {
        v = dp_v[i] / 10;
        ass = dp_v[i] % 2;
        if (walksat->cur_soln[v] != ass)
        {
            printf("%s walksat->cur_soln[v] != ass\n", qdpll->in_filename);
            exit(0);
        }
    }
}

void walksat_run_main(QDPLL* qdpll, WalkSAT* walksat, double _time_limit, double _strat)
{
    //return;
    //初始化walksat
    walksat_init(qdpll, walksat);

    if (walksat->use_hash == 1 && walksat->is_repeat == 1)
    {
        return;
    }

    //check_dp(qdpll, walksat);

    walksat_local_search(qdpll, walksat, _time_limit, _strat);

    if (walksat->unsat_stack_fill_pointer == 0)
    {
        if (walksat_verify_sol(qdpll, walksat) == 1) { walksat->satisfy_flag = 1; }
        else printf("error\n");
    }

    //printf("step = %d            walksat->satisfy_flag=%d\n", walksat->step, walksat->satisfy_flag);
}

void check_walksat_result(QDPLL* qdpll, WalkSAT* walksat)
{
    //printf("in check\n");
    //return;
    LitID lit;
    ...
    //check1: dpll中锁定的赋值，walksat不能改变
    for (vp = qdpll->assigned_vars, ve = qdpll->assigned_vars_top; vp < ve; vp++)
    {
        cur_num++;
        Var* v = VARID2VARPTR(qdpll->pcnf.vars, *vp);
        if (v->assignment == QDPLL_ASSIGNMENT_UNDEF)
        {
            printf("v->id=%d ***\n", v->id);
        }
        if (!QDPLL_VAR_ASSIGNED(v))
        {
            printf("!QDPLL_VAR_ASSIGNED(v)\n");
            exit(0);
        }
        if (QDPLL_VAR_ASSIGNED_TRUE(v))
        {
            if (walksat->cur_soln[v->id] != 1)
            {
                printf("walksat->cur_soln[v->id] != 1\n");
                printf("%d  %d(%d)  %d\n", v->id, walksat->cur_soln[v->id], walksat->dpll_fix[v->id], v->assignment);
                printf("%d  %d\n", num, cur_num);
                exit(0);
            }
        }
        else {
            if (walksat->cur_soln[v->id] != 0)
            {
                printf("walksat->cur_soln[v->id] != 0\n");
                printf("id=%d  type=%d  %d\n", v->id, walksat->var_type[v->id], v->mode);
                printf("%d  %d\n", walksat->fix[v->id], walksat->dpll_fix[v->id]);
                exit(0);
            }
        }
    }
    ...
    struct SAT_lit* clause_c;
    int v1, v;
    //检查sat_count=1的情况下，sat_var是不是对应子句里面的真文字
    for (int c = 0; c < walksat->num_clauses; c++)
    {
        if (walksat->clause_delete[c] > 0)  continue;
        ...
        for (struct SAT_lit* p = clause_c; (v = p->var_num) != 0; p++)
        {
            if (p->sense == walksat->cur_soln[v])//真文字
            {
                sat_num++;
                v1 = v;
            }
        ...
        //printf("\n");
        if (walksat->sat_count[c] != sat_num)
        {
            for (int i = 0; i < walksat->num_vc; i++)
            {
                printf("vc: %d %d\n", walksat->vc[i], walksat->vc_v[i]);
            }
            printf("%s  delete=%d  ", qdpll->in_filename, walksat->clause_delete[c]);
            printf("c=%d  walksat->sat_count[c]=%d  sat_num=%d\n", c, walksat->sat_count[c], sat_num);
            for (struct SAT_lit* p = clause_c; (v = p->var_num) != 0; p++)
            {
                if (p->sense == walksat->cur_soln[v])//真文字
                {
                ...
                printf("%d  walksat->cur_soln[v]=%d  p->sense=%d  %d  %d  %d  %d\n", v, walksat->cur_soln[v], p->sense, walksat->fix[v], walksat->dpll_fix[v],walksat->vc[v],walksat->vc_v[v]);
            }
            printf("\n");
            ...
            if (!qdpll->options.no_qbce_dynamic)//使用了动态子句消除，所以需要在搜索时，不能管阻塞子句
            {
                    ...
                    se = qdpll->qbcp_qbce_blocked_clauses.top; sp < se; sp++)
                {
                    ...
                    for (cp = stack.start, ce = stack.top; cp < ce; cp++)
                    {
                        ...
                        assert(_c->qbcp_qbce_blocked);
                        c = walksat->clause_qbf_to_sat[_c->id];
                        printf("c=%d  walksat->num_vars=%d\n", c, walksat->num_clauses);
        ...
        if (sat_num == 1 && walksat->sat_var[c] != v1)
        {
            printf("c=%d  walksat->sat_var[c]=%d  v1=%d\n", c, walksat->sat_var[c], v1);
            exit(0);
        }
    }
...
void check_sat_unit_assign_in_qcdcl(QDPLL* qdpll, WalkSAT* walksat)
{
    struct SAT_lit uc_lit;
    int uc_clause;
    int uc_var;
    ...
    for (walksat->unitclause_queue_beg_pointer = 0; walksat->unitclause_queue_beg_pointer < walksat->unitclause_queue_end_pointer; walksat->unitclause_queue_beg_pointer++)
    {
        ...
        v = VARID2VARPTR(qdpll->pcnf.vars, uc_var);
        assert(QDPLL_VAR_ASSIGNED(v));
        if (walksat->cur_soln[uc_var] == 1 && v->assignment != 1)
        {
            printf("%s walksat->cur_soln[uc_var] == 1 && v->assignment != 1\n");
            exit(0);
        }
        else if(walksat->cur_soln[uc_var] == 0 && v->assignment != -1) {
            printf("%s walksat->cur_soln[uc_var] == 0 && v->assignment != -1\n");
            exit(0);
            //assert(v->assignment == -1);
        }
    }
}

void Deep_Perturbation(QDPLL * qdpll, WalkSAT * walksat)
{
    num_dp = 0;
    int x;
    struct SAT_lit* clause_c;
    for (int i = 0, k, v, j; i <= walksat->unsat_stack_fill_pointer; i++)
    {
        j = walksat->unsat_stack[i];
        clause_c = walksat->clause_lit[j];
        x = 0;
        for (k = 0; k < walksat->clause_lit_count[j]; ++k)
        {
            v = clause_c[k].var_num;
            if (walksat->var_type[v] == 1 && walksat->dpll_fix[v] == 0 && walksat->fix[v] == 0)//v是存在变量
            {
                walksat->temp_list[x++] = v;
            }
        }
        if (x == 0)
        {
        ...
        k = rand() % x;
        v = walksat->temp_list[k];
        walksat->cur_soln[v] = 1 - walksat->cur_soln[v];

        //dp_v[num_dp] = v * 10 +  walksat->cur_soln[v];
        //dp[num_dp++] = j;
    }

}

void Fix_Forall_Variable(QDPLL* qdpll, WalkSAT* walksat)
{
    struct SAT_lit* clause_c;
    int x = 0;
    for (int i = 0, k, v, sence; i <= walksat->num_clauses; i++)
    {
        //printf("%d: %d\n", i, walksat->clause_to_variable_activity[i]);
        v = walksat->sat_var[i];
        if (walksat->var_type[v] == 1)  continue;//存在文字满足该子句，直接跳过该子句
        clause_c = walksat->clause_lit[i];
        for (k = 0; k < walksat->clause_lit_count[i]; ++k)
        {
            v = clause_c[k].var_num;
            sence = clause_c[k].sense;
            if (walksat->cur_soln[v] == sence)
            {
                if (walksat->var_type[v] == 1)  break;//存在文字满足该子句，直接跳过该子句
            }
        }
        if (k == walksat->clause_lit_count[i])
        {
            //到这里，说明子句i中的真文字，只有任意文字
            walksat->temp_list[x++] = i;
        }
    }
    if (x == 0)
    {
        printf("%s x=0 SAT!!!", qdpll->in_filename);

        double end1 = clock();
        qdpll->run_time = (end1 - qdpll->start) / CLOCKS_PER_SEC;
        //printf("%d\n", walksat->satisfy_flag);

        //check_walksat_result(qdpll, walksat);
        FILE* file = fopen("qbf_result.txt", "a");  // 打开文件用于写入

        // 方法1: fprintf
        fprintf(file, "%s SAT 1 %.6lf x=0\n", qdpll->in_filename, qdpll->run_time);
    ...
    int size = walksat_max(1, x * 0.2);
    walksat->num_vc = 0;
    for (int i = 0, k, c, sence, v; i < size; i++)
    {
        k = rand() % x;
        c = walksat->temp_list[k];
        walksat->temp_list[k] = walksat->temp_list[--x];
        clause_c = walksat->clause_lit[c];
        for (k = 0; k < walksat->clause_lit_count[c]; ++k)
        {
            v = clause_c[k].var_num;
            sence = clause_c[k].sense;
            if (walksat->cur_soln[v] == sence && walksat->dpll_fix[v] == 0 && walksat->fix[v] == 0)
            {
                if (walksat->var_type[v] == 1)
                {
                    printf("%s walksat->var_type[v] == 1 error\n", qdpll->in_filename);
                    printf("c = %d    walksat->clause_lit_count[c] = %d\n", c, walksat->clause_lit_count[c]);
                    for (int k1 = 0; k1 < walksat->clause_lit_count[c]; ++k1)
                    {
                        int v1 = clause_c[k1].var_num;
                        int sence1 = clause_c[k1].sense;
                        //if (walksat->cur_soln[v1] == sence1)
                        {
                            printf("%d(%d %d)\n", v1, sence1, walksat->cur_soln[v1]);
                        }
                    }
                    exit(0);
                }
                walksat->cur_soln[v] = 1 - walksat->cur_soln[v];
                walksat->dpll_fix[v] = 1;
                walksat->vc_v[walksat->num_vc] = walksat->cur_soln[v];
                walksat->vc[walksat->num_vc++] = v;
            }
        }
    }
    /*
    ...
    {
        printf("%d(%d) ",vc[i],vc_v[i]);
    }
    printf("\n");
    */
}

void assignment_variable_activity(QDPLL * qdpll, WalkSAT* walksat)
{
    struct SAT_lit* clause_c;
    int max_activity = -1;

    if (walksat->satisfy_flag == 1)
    {
        for (int i = 0, k, v, sence, u, num = 0; i < walksat->num_clauses; i++)
        {
            //printf("%d: %d\n", i, walksat->clause_to_variable_activity[i]);
            clause_c = walksat->clause_lit[i];
            for (k = 0; k < walksat->clause_lit_count[i]; ++k)
            {
                v = clause_c[k].var_num;
                sence = clause_c[k].sense;
                if (walksat->cur_soln[v] == sence)//满足该子句
                {
                    num++;
                }
            }
            ...
            {
                v = clause_c[k].var_num;
                //u = walksat->clause_sat_to_qbf[v];
                sence = clause_c[k].sense;
                if (walksat->cur_soln[v] == sence)//满足该子句
                {
                    //printf("%d  %d  %d  %d\n", u, v, qdpll->pcnf.size_vars, walksat->num_vars);
                    qdpll->variable_activity[v] += walksat_max(walksat->clause_to_variable_activity[i] / num, 1);
                    if (sence == 1)  qdpll->tend_ass_true[v] += walksat_max(walksat->clause_to_variable_activity[i] / num, 1);
                    else  qdpll->tend_ass_false[v] += walksat_max(walksat->clause_to_variable_activity[i] / num, 1);
                }
            }
        }
    }
    else {
        for (int i = 0, k, v, u, sence, num; i <= walksat->num_clauses; i++)
        {
            //printf("%d: %d\n", i, walksat->clause_to_variable_activity[i]);
            clause_c = walksat->clause_lit[i];
            num = walksat->clause_lit_count[i];
            for (k = 0; k < walksat->clause_lit_count[i]; ++k)
            {
                v = clause_c[k].var_num;
                sence = clause_c[k].sense;
    ...
    for (int i = 1; i <= walksat->num_vars; i++)
    {
        max_activity = walksat_max(max_activity, qdpll->variable_activity[i]);
    }
    
    for (int i = 1, u; i <= walksat->num_vars; i++)
    {
        if (walksat->fix[i] == 1)
        {
            //u = i;
            qdpll->variable_activity[i] = max_activity + 1;
        }
        //printf("%d(%d)  \n", i, qdpll->variable_activity[i]);
    }
    //exit(0);
    
}

void new_assignment_variable_activity(QDPLL* qdpll, WalkSAT* walksat)
{
    struct SAT_lit* clause_c;
    int max_activity = -1;
    /*
    if (walksat->satisfy_flag == 1)
    {
        for (int i = 0, k, v, sence, u, num = 0; i < walksat->num_clauses; i++)
        {
            ...
            {
                v = clause_c[k].var_num;
                if (walksat->is_collected[v] == 1)  continue;
                //u = walksat->clause_sat_to_qbf[v];
                sence = clause_c[k].sense;
                if (walksat->cur_soln[v] == sence)//满足该子句
                {
                    walksat->is_collected[v] = 1;
                }
            }
        }
        ...
        for (int v = 1; v <= walksat->num_vars; v++)
        {
            if (walksat->is_collected[v] == 1)
            {
                //printf("%d  %d  %d  %d\n", u, v, qdpll->pcnf.size_vars, walksat->num_vars);
                qdpll->variable_activity[v]++;
                walksat->is_collected[v] = 0;
                //if (sence == 1)  qdpll->tend_ass_true[v] += walksat_max(walksat->clause_to_variable_activity[i] / num, 1);
                //else  qdpll->tend_ass_false[v] += walksat_max(walksat->clause_to_variable_activity[i] / num, 1);
            }
        }
...
void check_vc(QDPLL * qdpll, WalkSAT* walksat)
{
    for (int i = 0; i < walksat->num_vc; i++)
    {
        int v = walksat->vc[i];
        int ass = walksat->vc_v[i];
        if (ass != walksat->cur_soln[v])
        {
            printf("%s error!!! ass != walksat->cur_soln[v]\n",qdpll->in_filename);
            exit(0);
        }
    }
}

void check_num_noassign_forall(QDPLL* qdpll)
{
    int num = 0, sum = 0, res;
    Var* v;
    VarID* vp;
    ...
    for (vp = qdpll->assigned_vars, ve = qdpll->assigned_vars_top; vp < ve; vp++)
    {
        // printf("vp=%d\n", *vp);
        v = VARID2VARPTR(qdpll->pcnf.vars, *vp);
        if (v->scope->type == QDPLL_QTYPE_FORALL)
        {
            num++;
        }
    }
    for (int i = 1; i < qdpll->pcnf.size_vars; i++)
    {
        v = VARID2VARPTR(qdpll->pcnf.vars, i);
        if (!v->scope)
        {
            //printf("!v->scope\n");
            continue;
    ...
    res = sum - num;
    if (res != qdpll->num_res_noassign_forall)
    {
        FILE* file = fopen("HybridQBF_error.txt", "a");  // 打开文件用于写入

        printf("res != qdpll->num_res_noassign_forall  %d  %d  %d  %d\n",sum, num, res, qdpll->num_res_noassign_forall);

        fprintf(file, "%s  res != qdpll->num_res_noassign_forall  error!!!\n", qdpll->in_filename);
...
solve (QDPLL * qdpll)
{
  ...
  qdpll->options.no_trivial_falsity = 0;//禁止掉trivial_falsity和trivial_truth
  qdpll->options.no_trivial_truth = 0;
  qdpll->options.no_dynamic_nenofex = 1;//关掉动态nenofex
  qdpll->options.no_empty_formula_watching = 0;//关闭空子句监视
  qdpll->options.no_qbce_dynamic = 0;

  //测试1，关闭动态子句消除，效果变差几十个
  //qdpll->options.no_qbce_dynamic = 1;

  //测试2,使用传统的QCDCL，而不是QPUP，BLOQQER少求解7个
  ...
  //测试4，--long-dist-res，使用长距离归结，bloqqerr差7个，hqspre差二十个
  //qdpll->options.long_dist_res = 1;

  //测试5，--no-lazy-qpup                 禁用懒惰 QPUP 风格的 QCDCL,BLOQQER差四个，HQSPRE好七个
  //qdpll->options.no_lazy_qpup = 1;

  //测试6，--no-pure-literals             禁用纯文字检测,都差100多个
  //qdpll->options.no_pure_literals = 1;

  //测试7，--no-empty-formula-watching    禁用空公式监视（默认启用）,总体差三个
  //qdpll->options.no_empty_formula_watching = 1;
  //qdpll->state.empty_formula_watcher_scheduled_update = 0;

  //测试8，--qbce-preprocessing           禁用 QBCE 预处理,和之前一样
  //qdpll->options.qbce_preprocessing = 0;
  ...
  //测试10，改变学习子句长度,增加两倍结果差不多，5倍10倍都差不多
  //qdpll->state.lcubes_size *= 10;
  //qdpll->state.lclauses_size *= 10;

  //测试12，改变学习子句删除方式
  //lbd小于等于2的不删除(86)，少61个
  //按照活跃度从小到大删除(87)，少46个
  //按照活跃度从小到大删除时，lbd小于等于2的不删除(89)，少65个
  ...
  //测试14，关闭QPUP+使用传统qcdcl+关闭qbce(83+84),少170左右
  //qdpll->options.traditional_qcdcl = qdpll->options.no_qpup_cdcl = qdpll->options.no_qpup_sdcl = 1;
  //qdpll->options.no_qbce_dynamic = 1;

  //测试15，禁止掉trivial_falsity和trivial_truth(85+86),基本一样
  //qdpll->options.no_trivial_falsity = 1;
  //qdpll->options.no_trivial_truth = 1;

  //测试16，关闭QPUP + 禁止掉trivial_falsity和trivial_truth(87+89)，少24
  //qdpll->options.traditional_qcdcl = qdpll->options.no_qpup_cdcl = qdpll->options.no_qpup_sdcl = 1;
  //qdpll->options.no_trivial_falsity = 1;
  //qdpll->options.no_trivial_truth = 1;

  //测试17，不使用依赖管理器（简化为按照量词顺序）（83+84）,总体少一个

  //测试18，禁掉trivial_falsity（85+86），总体多两个

  //关闭重启+全部为1开头(87+89),HQSPRE多几个，BLOQQER少很多

  //测试20，学习子句长度增加至子句长度*0.3,见20266（83）,HQSPRE少几个，BLOQQER少几十个

  //测试21，学习子句长度增加至子句长度*0.3，并保留lbd为2的子句(87)，HQSPRER少50个，BLOQQER少75个

  //测试22，不使用依赖管理器（简化为按照量词顺序）+ 关闭QPUP，HQSPRE少十几个，BLOQQE少60个
  //qdpll->options.traditional_qcdcl = qdpll->options.no_qpup_cdcl = qdpll->options.no_qpup_sdcl = 1;

  //测试23，不使用cube，即存储cube长度的数据结构永远限制为1,HQSPR少40个，BLOQQER少90个

  //测试24，不使用clause

  //！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！再测试，就不使用随机数了
  // use_ls_to_find_true_after_forall_assign已设置为0
  // 
  // 原版(84)  660 + 542
  // 
  //测试1，开启动态nenofex(86)  649 + 534
  //qdpll->options.no_dynamic_nenofex = 0;

  //测试2,使用传统的QCDCL，而不是QPUP(87)  652 + 525
  //qdpll->options.traditional_qcdcl = qdpll->options.no_qpup_cdcl = qdpll->options.no_qpup_sdcl = 1;

  //测试3，--no-lazy-qpup(89)  667 + 552
  //qdpll->options.no_lazy_qpup = 1;
  
  //测试4，关闭空子句监视(83) 661 + 540
  //qdpll->options.no_empty_formula_watching = 1;//关闭空子句监视

  //测试5，关闭重启，(84)  641 + 519
  //内重启扩大两倍，(84+86)664+549
  //内重启扩大五倍，
  //内外重启均扩大2倍，(85 HQSPRE,86BLOQQER) 657 + 550

  //测试6，禁止掉trivial_falsity和trivial_truth,(87+89)660+537
  //qdpll->options.no_trivial_falsity = 1;
  //qdpll->options.no_trivial_truth = 1;

  //测试6，禁止掉trivial_falsity,(87+89),655 + 554
  //qdpll->options.no_trivial_falsity = 1;

  //测试7，学习子句长度增加至子句长度*0.3,见20307（85+86）, 657 + 545

  //测试8，学习子句长度增加至子句长度*0.3，并保留lbd为2的子句(87+89)，625 + 509
  ...
  double TimeLimit = 900, start = clock();
  qdpll->start = start;
  QDPLLMemMan* mm = qdpll->mm;
  double end;

  //初始化glue用的数组
  qdpll->glue_decide_level = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->variable_activity = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->tend_ass_true = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->tend_ass_false = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->backtrack_state_SAT = (int**)qdpll_malloc(mm, 2 * sizeof(int*));
  qdpll->backtrack_state_SAT[0] = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->backtrack_state_SAT[1] = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->backtrack_state_UNSAT = (int**)qdpll_malloc(mm, 2 * sizeof(int*));
  qdpll->backtrack_state_UNSAT[0] = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->backtrack_state_UNSAT[1] = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->_polarity = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->_polarity_LS = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->vis = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->back_need_collect = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->tail_saving = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->is_decision_variable = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  //qdpll->hope = (int**)qdpll_malloc(mm, 2 * sizeof(int*));
  //qdpll->hope[0] = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll->hope = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));

  qdpll->max_num_undeletable = 1000005;
  qdpll->undeletable = (int*)qdpll_malloc(mm, (qdpll->max_num_undeletable + 1) * sizeof(int));
  qdpll->undeletable_array = (_Node*)qdpll_malloc(mm, (qdpll->max_num_undeletable + 1) * sizeof(_Node));
  
  for (int i = 0; i <= qdpll->max_num_undeletable; i++)
  {
      qdpll->undeletable[i] = 0;
  }
  
  qdpll->num_tail_saving = 0;
  qdpll->current_state = 0;//undef
  qdpll->sort_time = 0;
  qdpll->delete_num = 0;

  for (int i = 0; i <= qdpll->pcnf.size_vars; i++)
  {
      qdpll->glue_decide_level[i] = 0;
      qdpll->variable_activity[i] = 0;
      qdpll->tend_ass_true[i] = 0;
      qdpll->tend_ass_false[i] = 0;
      qdpll->backtrack_state_SAT[0][i] = 0;
      qdpll->backtrack_state_SAT[1][i] = 0;
      qdpll->backtrack_state_UNSAT[0][i] = 0;
      qdpll->backtrack_state_UNSAT[1][i] = 0;
      qdpll->_polarity[i] = 2;
      qdpll->_polarity_LS[i] = 2;
      qdpll->vis[i] = 0;
      qdpll->back_need_collect[i] = 0;
      qdpll->tail_saving[i] = 0;
      qdpll->is_decision_variable[i] = 0;
      //qdpll->hope[0][i] = qdpll->hope[1][i] = 0;
      qdpll->hope[i] = 0;
  }
  qdpll->glue1 = qdpll->glue2 = qdpll->glue3 = qdpll->glue4 = qdpll->glue5 = qdpll->glue6 = 0;
  for (int i = 0; i <= 100; i++)
  {
      qdpll->glueg[i] = 0;
  }

  //printf("mm->cur_allocated = %d  111\n", mm->cur_allocated);
  //WalkSAT* walksat = (WalkSAT*)qdpll_malloc(mm, sizeof(WalkSAT));
  //printf("mm->cur_allocated = %d  222\n", mm->cur_allocated);
  WalkSAT* walksat = (WalkSAT*)qdpll_malloc(mm, sizeof(WalkSAT));
  //qdpll->glue_decide_level = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_vars + 1) * sizeof(int));

  //********参数
  int max_flip_upbound = 1000000;
  //max_flip_upbound = 500000;//设置翻转次数上限，防止ls花费过多时间,当前最好
  max_flip_upbound = 100000;
  //max_flip_upbound = 50000;
  //max_flip_upbound = 10000;
  //max_flip_upbound = 999999999;
  int max_flip = 10 * qdpll->pcnf.size_vars;
  if (max_flip > max_flip_upbound)  max_flip = max_flip_upbound;

  int use_init_zero = 0;

  //********策略开关
  int follow_ls_assignment = 0;//qcdcl的赋值使用ls的值
  int use_variable_activity = 0;
  qdpll->use_variable_activity = use_variable_activity;
  walksat->use_variable_activity = use_variable_activity;
  qdpll->variable_activity_decay_ratio = 50;
  qdpll->num_backtrack_zero = 0;
  qdpll->ls_find_true = qdpll->ls_find_false = 0;

  //回溯后，如果当前是存在文字，可以使用ls探索一个满足分支
  int use_ls_to_find_true = 0;
  double call_ls_ratio = 0.6;//剩余变量大于百分之use_ratio变量赋值才使用ls
  int call_ls_num = 0;//调用ls的次数
  qdpll->call_ls_num = 0;
  qdpll->all_call_ls_num = 0;
  int call_ls_num_threshold = 10;
  int call_ls_num_fail = 0;//连续失败的次数
  int assign_num_before_while = 0;

  //所有的任意变量都赋值完之后，如果剩余存在变量还剩一定的数量，暂定所有变量的20%以上，使用ls快速求解一个true
  int use_ls_to_find_true_after_forall_assign = 1;
  qdpll->use_ls_to_find_true_after_forall_assign = use_ls_to_find_true_after_forall_assign;
  qdpll->num_res_noassign_forall = 0;
  qdpll->time_of_call_ls = 0;
  double time_of_call_ls_bound = 50;//调用LS的最大时间和
  double basic_activity_multiplicative_factor = 10;
  qdpll->ls_r = 0;

  //使用Hash
  int use_hash = 1;
  if (use_ls_to_find_true_after_forall_assign == 0)  use_hash = 0;
  walksat->use_hash = use_hash;
  walksat->size_hash = 100000;

  //在当前回溯之后需要找到false时，让任意文字多满足子句，存在文字少满足子句
  //depqbf一直用的是，任意文字少满足子句，存在文字多满足子句
  int use_individual_heuristic = 1;
  qdpll->use_individual_heuristic = use_individual_heuristic;
  qdpll->branch_in_forall = 0;
  int vid;
  int assignment_strategy_change_ratio = 50;
  //相位保存策略
  int use_phase_saving = 1;

  //学习子句和cube删除
  int use_clause_cube_delete = 0;
  qdpll->use_clause_cube_delete = use_clause_cube_delete;
  qdpll->use_clause_cube_delete_after_restart = 1;
  qdpll->clause_move = 0;

  //相位重启
  qdpll->use_rephase = 0;
  int now_learn_size, new_learn_size;
  qdpll->just_start = 0;

  //不采用qcdcl给ls传递赋值，ls初始化全部随机
  qdpll->no_qcdcl_guide_ls = 0;

  qdpll->only_phase_saving = 0;
  if (qdpll->only_phase_saving == 1)
  {
      for (int i = 0; i <= qdpll->pcnf.size_vars; i++)
      {
          //if(rand()%100 < 50)  qdpll->_polarity[i] = -1;
          //else  qdpll->_polarity[i] = 1;
          qdpll->_polarity[i] = 1;
      }
  }


  //qdpll->options.dh = QDPLL_DH_SIMPLE;//变量赋值方式

  int use_sat_unit = 0;

  int xxx = 1000000;
  qdpll->options.trivial_truth_decision_limit = xxx;
  qdpll->options.trivial_falsity_decision_limit = xxx;
  qdpll->num_false = qdpll->num_true = qdpll->num_undef = 0;
  //qdpll->num_aaa = 0;

  //printf("init\n");
  
  //init_walksat(qdpll, walksat);
  init_walksat1(qdpll, walksat);
  qdpll->belong = (int*)qdpll_malloc(mm, (qdpll->pcnf.size_clause + 1) * sizeof(int));
  for (int i = 0; i <= qdpll->pcnf.size_clause; i++)
  {
      qdpll->belong[i] = 0;
  }
  ...
  //printf("build\n");
  //将子句传给walksat
  build_walksat_clause(qdpll, walksat, use_sat_unit);

  //if (walksat->ratio >= 4)  qdpll->options.no_qbce_dynamic = 1;//*****

  if (walksat->satisfy_flag == -1)
  {
      //printf("walksat->satisfy_flag == -1  111\n");
      free_walksat(qdpll, walksat);
      return QDPLL_RESULT_UNSAT;//原子句冲突，返回UNSAT
  }
  ...
  //printf("before sat bcp\n");
  /*
  if (use_ls_to_find_true_after_forall_assign)
  {
      for (int i = 1; i < qdpll->pcnf.size_vars; i++)
      {
          Var* _v = VARID2VARPTR(qdpll->pcnf.vars, i);
          if (!_v->scope)
          {
              //printf("!v->scope\n");
              continue;
          }
          if (_v->scope->type == QDPLL_QTYPE_FORALL)
          {
              //printf("sum = %d\n", sum);
              qdpll->num_res_noassign_forall++;
          }
      }
      int _nums = 0;
      for (int ii = 1; ii <= walksat->num_vars; ii++)
      {
          if (walksat->var_type[ii] == 0)
          {
              _nums++;
          }
      }

      if (qdpll->num_res_noassign_forall != _nums)
      {
          FILE* file = fopen("HybridQBF_error.txt", "a");  // 打开文件用于写入

          printf("qdpll->num_res_noassign_forall != _nums  %d  %d\n", qdpll->num_res_noassign_forall, _nums);

          fprintf(file, "%s  qdpll->num_res_noassign_forall != _nums  %d  %d  error!!!\n", qdpll->in_filename, qdpll->num_res_noassign_forall, _nums);
  ...
  //assert(walksat->unitclause_queue_end_pointer == 0);
  //assert(walksat->simplify == 0);

  //printf("walksat->unitclause_queue_end_pointer = %d\n", walksat->unitclause_queue_end_pointer);

  if (walksat->unitclause_queue_end_pointer > 0) walksat_preprocess(walksat);

  if (walksat->satisfy_flag == -1)
  {
      //printf("walksat->satisfy_flag == -1  222\n");
      free_walksat(qdpll, walksat);
      return QDPLL_RESULT_UNSAT;//原子句冲突，返回UNSAT
  }
  ...
  walksat_build_neighbor_relation(qdpll, walksat);

  //设置walksat运行需要的一些参数
  walksat_set_parameter(walksat, max_flip);
  //初次给walksat赋值,因为后面每次搜索都是使用前一次搜索的最后赋值开始的
  first_walksat_assignment(walksat);
  //printf("---\n");
  
  int num_ls_call = 0, max_num_ls_call = 100, sum_ls_flip = 0;
  int iter_ls_flip = max_flip / 5;
  if (use_variable_activity == 1)
  {
      walksat->max_flips = iter_ls_flip;

      while (sum_ls_flip < max_flip * 10 && num_ls_call < max_num_ls_call)
      {
          //check_vc(qdpll, walksat);
          //printf("111\n");
          walksat_run_main(qdpll, walksat, 100, clock());

          end = clock();
          if (((end - qdpll->start) / CLOCKS_PER_SEC) > 100)
          {
              break;
          }
          ...
          //check_walksat_result(qdpll, walksat);
          //printf("222.5\n");
          //assignment_variable_activity(qdpll, walksat);
          new_assignment_variable_activity(qdpll, walksat);
          //printf("333\n");
          num_ls_call++;
          sum_ls_flip += walksat->step;
          //printf("%d: %d  %d\n", num_ls_call, walksat->satisfy_flag, sum_ls_flip);
          walksat->num_vc = 0;

          switch (walksat->satisfy_flag) {
          case 1:
              // 找到可行解，先寻找所有仅被任意变量满足的子句，再选取一部分子句翻转这些任意变量，并固定这些任意变量
              //printf("444.1\n");
              Fix_Forall_Variable(qdpll, walksat);
              //printf("444.2\n");
              break;
          case 0:
              // 未找到可行解,进行一个大扰动，对于每个为满足的子句，强制随机选择一个存在变量满足该子句
              //printf("444.3\n");
              Deep_Perturbation(qdpll, walksat);
              //printf("444.4\n");
              break;
          default:
              // -1，应该不存在这样的情况
  ...
  //printf("walksat->satisfy_flag = %d *-*-* +++\n", walksat->satisfy_flag);

  //printf("qdpll->options.dh = %d\n", qdpll->options.dh);

#ifndef NDEBUG
  check_walksat_result(qdpll, walksat);
#endif

  //walksat_set_parameter(walksat, max_flip / 10);
  //printf("walksat->unitclause_queue_end_pointer = %d\n", walksat->unitclause_queue_end_pointer);

  //if (walksat->unitclause_queue_end_pointer > 0)  backtrack_sat_bcp(walksat);

  //printf("walksat->satisfy_flag = %d *-*-*\n", walksat->satisfy_flag);

  //不要SAT的bcp了，全部使用QBF的bcp
  //if (walksat->unitclause_queue_end_pointer > 0)  backtrack_sat_bcp(walksat);
  ...
  if (0)
  {
      ...
      double ttt = ((end - qdpll->start) / CLOCKS_PER_SEC);
      printf("ttt = %.2lf\n", ttt);
  }
  ...
  //qdpll->state.lcubes_size = walksat->num_clauses / 3;
  //qdpll->state.lclauses_size = walksat->num_clauses / 3;
  //printf("in while\n");
  //qdpll->state.lcubes_size *= 2;
  //qdpll->state.lclauses_size *= 2;
  //printf("qdpll->state.lcubes_size = %d\n", qdpll->state.lcubes_size);
  //qdpll->state.lclauses_size /= 2;
  //qdpll->state.lcubes_size += qdpll->state.lcubes_size * 0.3;
  //qdpll->state.lclauses_size = qdpll->state.lclauses_size * 1.2;
  //qdpll->state.lcubes_size = qdpll->state.lcubes_size * 1.2;
  //qdpll->state.lclauses_size -= 1;
  //qdpll->state.lcubes_size -= 1;
  if (0)
  {
      end = clock();
      qdpll->run_time = (end - start) / CLOCKS_PER_SEC;
      printf("runtime = %.6lf\n", qdpll->run_time);
  }
  //printf("111\n");
  if (use_init_zero == 1)
  {
      ...
      for (int v = 1; v <= walksat->num_vars; v++)
      {
          _temp_v = LIT2VARPTR(qdpll->pcnf.vars, v);
          if (!_temp_v);
          else
          {
              _temp_v->priority = 0;
              if (_temp_v->priority_pos != QDPLL_INVALID_PQUEUE_POS)
              {
                  var_pqueue_increase_key(qdpll, _temp_v->id);
                  printf("var_pqueue_increase_key(qdpll, _temp_v->id);\n");
              }
          }
          //_temp_v->priority = 0;
      }
  ...
  QDPLLAssignment lit_value;
  unsigned int lit_mode;
  int just_backtrack = 1;
  int first_bcp = 0;

  if (use_ls_to_find_true)
  {
      walksat_run_main(qdpll, walksat, 5, clock());
      call_ls_num++;
      qdpll->call_ls_num = call_ls_num;
      if (walksat->satisfy_flag == 1)
      {
          call_ls_num_fail = 0;
          follow_ls_assignment = 1;
      }
      else
      {
          call_ls_num_fail++;
          if (call_ls_num_fail > call_ls_num_threshold)
          {
              //连续多次未成功，就关闭ls
              use_ls_to_find_true = 0;
          }
      }
  }

  assign_num_before_while = qdpll->assigned_vars_top - qdpll->assigned_vars;
  //printf("assign_num_before_while=%d  qdpll->pcnf.size_vars=%d\n", assign_num_before_while, qdpll->pcnf.size_vars);
  /*
  char* r11 = strstr(qdpll->in_filename, "incrementer-enc08-uniform-depth-33.qdimacs");
  ...
  if (r11 != NULL)
  {
      while (1)
      {
          ...
          qdpll->run_time = (end - start) / CLOCKS_PER_SEC;
          if (qdpll->run_time > TimeLimit)
          //if (qdpll->run_time > 30)
          {
  ...
  while (1)
    {
      ...
      {
          Var* temp_v;
          double ave_priority = 0;
          for (int i = 1; i <= walksat->num_vars; i++)
          {
              temp_v = LIT2VARPTR(qdpll->pcnf.vars, i);
              //printf("%d %.2lf\n", temp_v->id, temp_v->priority);
              ave_priority += temp_v->priority / walksat->num_vars;
          }
          printf("ave_priority = %.2lf\n", ave_priority);
      }

      if (use_ls_to_find_true_after_forall_assign && 0)  check_num_noassign_forall(qdpll);
      /*
      end = clock();
      qdpll->run_time = (end - start) / CLOCKS_PER_SEC;
      if (qdpll->run_time > TimeLimit)
      ...
      if (first_bcp == 1)
      {
          assign_num_before_while = qdpll->assigned_vars_top - qdpll->assigned_vars;
          //printf("assign_num_before_while=%d  qdpll->pcnf.size_vars=%d\n", assign_num_before_while, qdpll->pcnf.size_vars);
          //check_sat_unit_assign_in_qcdcl(qdpll, walksat);
      }
      //printf("after bcp\n");
...
#endif
                  ccadical_assume (qdpll->trivial_falsity_solver, QDPLL_VAR_ASSIGNED_TRUE (v) ? v->id : -v->id);
                }
            }
...
#ifndef NDEBUG
              //assert_trivial_falsity_assumption_unsat_test(qdpll, qdpll->state.unsat_branch_clause);
#endif
              /* Set solver state to UNSAT so that we enter the respective
                 branch below. There, a new clause will be learned using
              ...
              if (QDPLL_VAR_FORALL (v))
                {
                  ...
                  ccadical_assume (qdpll->trivial_truth_solver,
              ...
              for (c = qdpll->pcnf.clauses.first; c; c = c->link.next)
                {
                  assert (c->trivial_truth_selector);
                  if (c->qbcp_qbce_blocked || is_clause_satisfied (qdpll, c))
                      ccadical_assume (qdpll->trivial_truth_solver, c->trivial_truth_selector);
                  else
                      ccadical_assume (qdpll->trivial_truth_solver, -c->trivial_truth_selector);
                }
...
#endif
          //printf("qdpll->options.trivial_truth_decision_limit = %d\n", qdpll->options.trivial_truth_decision_limit);
          /*
          int sat_result = ccadical_sat (qdpll->trivial_truth_solver,
					qdpll->options.trivial_truth_decision_limit > 0 ? (int) qdpll->options.trivial_truth_decision_limit : -1);*/
          ccadical_limit(qdpll->trivial_truth_solver, "decisions",
              qdpll->options.trivial_falsity_decision_limit > 0 ? (int)qdpll->options.trivial_falsity_decision_limit : -1);
          int sat_result = ccadical_sat(qdpll->trivial_truth_solver);
          //printf("3.5\n");
#if COMPUTE_STATS
      ...
      if (state == QDPLL_SOLVER_STATE_UNSAT)
        {
          follow_ls_assignment = 0;
          qdpll->just_start = 0;

          qdpll->num_false++;
          //printf("UNSAT\n");
          //printf("Conflict begin!\n");
          ...
          now_learn_size = qdpll->pcnf.learnt_clauses.cnt;
          backtrack_level = analyze_conflict(qdpll);
          new_learn_size = qdpll->pcnf.learnt_clauses.cnt;

          if (qdpll->use_rephase && (new_learn_size - now_learn_size == 1))
          {
              modify_qsscore(qdpll, qdpll->pcnf.learnt_clauses.first, QDPLL_QTYPE_EXISTS);
          }
          /*
          int _num = qdpll->state.clause_resizes;
          int _iter = 10;
          qdpll->delete_mode = 101;

          if (qdpll->use_clause_cube_delete == 1 && (_num % _iter == 0 && _num > 0))
          {
              qdpll->delete_mode = rand() % 100;//学习子句删除模式
          }
          check_resize_learnt_constraints(qdpll, QDPLL_QTYPE_EXISTS);
          ...
          */
          ...
          if (qdpll->use_individual_heuristic == 1 && backtrack_level != -1)
          {
              if (!qdpll->state.forced_assignment.var)
              {
                  printf("%s !qdpll->state.forced_assignment.var 111\n",qdpll->in_filename);
                  exit(0);
              }
              vid = qdpll->state.forced_assignment.var->id;
              int val = (qdpll->state.forced_assignment.assignment + 1) / 2;
              qdpll->skip_variable = vid;
              use_phase_saving = 1;
              /*
              if (qdpll->is_time_exit)//退出
              {
                  qdpll->record_decide_level_conf = qdpll->state.decision_level;
                  qdpll->record_ass_num_conf = qdpll->assigned_vars_top - qdpll->assigned_vars;
                  printf("%s %d %d %d %d\n", qdpll->in_filename, 
                      qdpll->record_decide_level_conf, qdpll->record_ass_num_conf,
                      qdpll->record_decide_level_solu, qdpll->record_ass_num_solu);
                  exit(0);
              }*/
              //当前回溯的这个变量一定是存在变量
              /*
              ...
              //printf("UNSAT: %10d  %10d  %10d  %10d  %10d\n", vid, val, qdpll->branch_in_forall, qdpll->backtrack_state_SAT[val][vid], qdpll->backtrack_state_UNSAT[val][vid]);
              
              //最初版，任意变量之下找UNSAT，存在变量之下找SAT
              //qdpll->branch_in_forall = 0;

              //与最初版相反，任意变量之下找SAT，存在变量之下找UNSAT
              //qdpll->branch_in_forall = 1;

              /*
              //无概率
              if (qdpll->branch_in_forall == 0)
              {
                  //这代表之前找到的false是由，任意文字少满足子句的策略找到的，
                  //现在右分找到false的概率应该较大，因此可以直接采用任意文字多满足子句，存在文字少满足子句的策略
                  qdpll->branch_in_forall = 1;
              }
              else if ((qdpll->backtrack_state_SAT[val][vid] > qdpll->backtrack_state_UNSAT[val][vid])) {
                  //之前的false是由“任意文字多满足子句找到的”
                  //现在如果过去找到SAT的次数比UNSAT的次数多，那么就可以使用“任意文字少满足子句”
                  //去寻找一个SAT
                  qdpll->branch_in_forall = 0;
              }
              */
              ...
              {
                  //之前的false是由“任意文字多满足子句找到的”
                  //用过去找到SAT的次数比UNSAT的次数，来实现一个概率选择
                  int i = qdpll->backtrack_state_SAT[val][vid] + qdpll->backtrack_state_UNSAT[val][vid], j;
                  if (i > 0) 
                  {
                      j = qdpll->backtrack_state_SAT[val][vid] * 100 / i;
                      if(rand()%100 < j)  qdpll->branch_in_forall = 0;
                  }
              }*/
              /*
              //使用新概率
              ...
              else
              {
                  //之前的false是由“任意文字多满足子句找到的”
                  //用过去找到SAT的次数比UNSAT的次数，来实现一个概率选择
                  if (qdpll->backtrack_state_SAT[val][vid] > qdpll->backtrack_state_UNSAT[val][vid])
                  {
                      int i = qdpll->backtrack_state_SAT[val][vid] + qdpll->backtrack_state_UNSAT[val][vid], j;
                      j = (qdpll->backtrack_state_SAT[val][vid] - qdpll->backtrack_state_UNSAT[val][vid]) * 100 / i;
                      if (rand() % 100 < j)  qdpll->branch_in_forall = 0;
                  }
              }*/
              /*
              ...
              else
              {
                  ...
                  if (qdpll->backtrack_state_SAT[val][vid] > qdpll->backtrack_state_UNSAT[val][vid])
                  {
                      if (rand() % 100 < assignment_strategy_change_ratio)  qdpll->branch_in_forall = 0;
                  }
              }
              */
              //qdpll->back_need_collect[vid] = 1;
          }
          ...
          if (backtrack_level == QDPLL_INVALID_DECISION_LEVEL)
          ...
          else
            {
              ...
              if (!check_and_restart (qdpll, backtrack_level, walksat))
                {
#if COMPUTE_STATS
      ...
      else if (state == QDPLL_SOLVER_STATE_SAT)
        {
          ...
          now_learn_size = qdpll->pcnf.learnt_cubes.cnt;
          backtrack_level = analyze_solution(qdpll);
          new_learn_size = qdpll->pcnf.learnt_cubes.cnt;

          if (qdpll->use_rephase && (new_learn_size - now_learn_size == 1))
          {
              modify_qsscore(qdpll, qdpll->pcnf.learnt_cubes.first, QDPLL_QTYPE_FORALL);
          }

          /*
          int _num = qdpll->state.cube_resizes;
          int _iter = 50;
          qdpll->delete_mode = 101;
          ...
          */
          ...
          //printf("qdpll->num_res_noassign_forall = %d\n", qdpll->num_res_noassign_forall);

          if (qdpll->use_individual_heuristic == 1 && backtrack_level != -1)
          {
              if (!qdpll->state.forced_assignment.var)
              {
                  printf("!qdpll->state.forced_assignment.var 222    backtrack_level=%d  ***\n", backtrack_level);
                  exit(0);
              }
              vid = qdpll->state.forced_assignment.var->id;
              int val = (qdpll->state.forced_assignment.assignment + 1) / 2;
              ...
              if (qdpll->is_time_exit)//退出
              {
                  qdpll->record_decide_level_solu = qdpll->state.decision_level;
                  qdpll->record_ass_num_solu = qdpll->assigned_vars_top - qdpll->assigned_vars;
                  printf("%s %d %d %d %d\n", qdpll->in_filename,
                      qdpll->record_decide_level_conf, qdpll->record_ass_num_conf,
                      qdpll->record_decide_level_solu, qdpll->record_ass_num_solu);
                  exit(0);
              ...
              //printf("  SAT: %10d  %10d  %10d  %10d  %10d\n", vid, val, qdpll->branch_in_forall, qdpll->backtrack_state_SAT[val][vid], qdpll->backtrack_state_UNSAT[val][vid]);
              ...
              /*
              //无概率
              if (qdpll->branch_in_forall == 1)
              {
                  //这代表之前找到的true是由，任意文字多满足子句的策略找到的，
                  //现在右分找到true的概率应该较大，因此可以直接采用任意文字少满足子句，存在文字多满足子句的策略
                  qdpll->branch_in_forall = 0;
              }
              else if ((qdpll->backtrack_state_SAT[val][vid] < qdpll->backtrack_state_UNSAT[val][vid])) {
                  //之前的SAT是由“存在文字多满足子句找到的”
                  //现在如果过去找到UNSAT的次数比SAT的次数多，那么就可以使用“任意文字多满足子句”
                  //去寻找一个UNSAT
                  qdpll->branch_in_forall = 1;
              }*/
              ...
              else
              {
                  ...
                  {
                      j = qdpll->backtrack_state_SAT[val][vid] * 100 / i;
                      if (rand() % 100 >= j)  qdpll->branch_in_forall = 1;
                  }
              }*/
              /*
              //使用新概率
              ...
              else
              {
                  ...
                  {
                      int i = qdpll->backtrack_state_SAT[val][vid] + qdpll->backtrack_state_UNSAT[val][vid], j;
                      j = (qdpll->backtrack_state_UNSAT[val][vid] - qdpll->backtrack_state_SAT[val][vid]) * 100 / i;
                      if (rand() % 100 < j)  qdpll->branch_in_forall = 1;
                  }
              }
              */
              ...
              else
              {
                  ...
                  if (qdpll->backtrack_state_SAT[val][vid] < qdpll->backtrack_state_UNSAT[val][vid])
                  {
                      if (rand() % 100 < assignment_strategy_change_ratio)  qdpll->branch_in_forall = 1;
                  }
              }*/
          }
      ...
      else
        {
          ...
          if (just_backtrack && 0)
          {
              //assignment_num = qdpll->assigned_vars_top - qdpll->assigned_vars;
              //walksat->max_flips = (walksat->num_vars - assignment_num) * 10;//剩余未赋值变量个数
              //printf("in j b\n");
              walksat_run_main(qdpll, walksat, 10, clock());
              //printf("out j b\n");
#ifndef NDEBUG
              check_walksat_result(qdpll, walksat);
#endif
          ...
          /*
          if (qdpll->num_res_noassign_forall == 0)
          {
              printf("qdpll->num_res_noassign_forall == 0\n");
          }
          int xxx = qdpll->pcnf.size_vars - 1 - (qdpll->assigned_vars_top - qdpll->assigned_vars);
          printf("qdpll->num_res_noassign_forall = %d    res = %d\n", qdpll->num_res_noassign_forall,xxx);
          */

          //if (use_ls_to_find_true_after_forall_assign && just_backtrack && qdpll->num_res_noassign_forall == 0)
          if (use_ls_to_find_true_after_forall_assign && just_backtrack)
          //if (use_ls_to_find_true_after_forall_assign)
          {
              //使用ls + 刚刚回溯之后 + 任意变量全部已经赋值
              int already_ass = qdpll->assigned_vars_top - qdpll->assigned_vars;
              double res_ratio = (double)(walksat->num_vars - already_ass) / (walksat->num_vars);//未赋值变量个数
              //printf("res_ratio = %.2lf\n", res_ratio);
              //if (res_ratio > call_ls_ratio && (qdpll->state.cnt_state_undetermined_after_qbcp >> qdpll->call_ls_num) > 0)
              if (res_ratio > call_ls_ratio && (qdpll->ls_r >> qdpll->call_ls_num) > 0)
              {
                  //已赋值变量个数要小于call_ls_ratio
                  struct SAT_lit* clause_c;
                  Var* temp_v;
                  int max_vis = -1;
                  double add_priority;
                  ...
                  /*
                  //if ((1 << qdpll->call_ls_num) > qdpll->ls_r)
                  if ((1 << call_ls_num) > qdpll->state.num_backtracks)
                  {
                      FILE* file = fopen("HybridQBF_error.txt", "a");  // 打开文件用于写入

                      fprintf(file, "%s (1 << num_ls_call) > ls_r  error!!!\n", qdpll->in_filename);
                  ...
                  walksat_run_main(qdpll, walksat, 10, clock());
                  /*
                  if (walksat->is_repeat == 1)
                  {
                      printf("%s hash is repeat\n",qdpll->in_filename);
                  }*/

                  if (walksat->is_repeat == 0)
                  {
                      //qdpll->ls_call_numbers++;
                      qdpll->call_ls_num++;
                      qdpll->all_call_ls_num++;
                      //printf("qdpll->all_call_ls_num = %d\n", qdpll->all_call_ls_num);
                      double tt2 = clock();
                      double ttt = (tt2 - tt1) / CLOCKS_PER_SEC;
                      if (ttt > 10)
                      {
                          use_ls_to_find_true_after_forall_assign = 0;//单词调用超过10s
                      }
                      qdpll->time_of_call_ls += ttt;
                      //qdpll->ls_call_time += ttt;
                      //printf("walksat->satisfy_flag = %d\n", walksat->satisfy_flag);
                      ...
                      */
                      ...
                      if (walksat->satisfy_flag == 1)
                      {
                          ...
                          for (int i = 0, k, v, sence, u, num = 0; i < walksat->num_clauses; i++)
                          {
                              ...
                              for (k = 0; k < walksat->clause_lit_count[i]; ++k)
                              {
                                  ...
                                  {
                                      qdpll->vis[v]++;
                                      //max_vis = walksat_max(max_vis, qdpll->vis[v]);
                                  }
                              }
                          }
                          ...
                          for (int v = 1; v <= walksat->num_vars; v++)
                          {
                              if (qdpll->vis[v] > 0)
                              {
                                  temp_v = LIT2VARPTR(qdpll->pcnf.vars, v);
                                  add_priority = qdpll->vis[v] * 1.0 / max_vis * ave_priority;
                                  //add_priority = qdpll->vis[v] * 1.0 / walksat->num_clauses * ave_priority;
                                  temp_v->priority += add_priority;
                                  if (temp_v->priority_pos != QDPLL_INVALID_PQUEUE_POS)
                                      var_pqueue_increase_key(qdpll, temp_v->id);
                                  qdpll->vis[v] = 0;
                                  //printf("%d %.2lf %.2lf\n", v, add_priority, temp_v->priority);
                              }
                          }//*/
                          //printf("qdpll->state.cnt_state_undetermined_after_qbcp = %d\n", qdpll->state.cnt_state_undetermined_after_qbcp);
                          //exit(0);

                      }
                      else if (use_ls_to_find_true_after_forall_assign == 1)
                      {
                          qdpll->ls_find_false++;
                          max_vis = 0;
                          ///*!!!!!!!!!!!!!rebuttal
                          for (int i = 0, k, v, j; i < walksat->num_clauses; i++)
                          {
                              if (walksat->record_priority[i] > 0)//在walksat_init中置零
                              {
                                  clause_c = walksat->clause_lit[i];
                                  for (k = 0; k < walksat->clause_lit_count[i]; ++k)
                                  {
                      ...
                      //qdpll->update_time += (clock() - ttt3) / CLOCKS_PER_SEC;

                      if (qdpll->time_of_call_ls > time_of_call_ls_bound)
                      {
                          use_ls_to_find_true_after_forall_assign = 0;
                      }
                      just_backtrack = 0;
                  }
              }
          ...
          //printf("decision_var->id = %d\n", decision_var->id);

          if (first_bcp == 1)
          {
              int vid = decision_var->id;
              //qdpll->branch_in_forall = 1 - walksat->var_type[vid];
              //qdpll->branch_in_forall = walksat->var_type[vid];//开头第一个如果是存在文字，尽量使存在文字满足更多的子句，这与回溯后的存在文字不同
              //qdpll->first_branch_in_forall = qdpll->branch_in_forall;
          }
          
          //if ( (follow_ls_assignment || 1) && 0 )   
          //assignment = qdpll->_polarity[decision_var->id];//直接使用相位保存
          //两阶段相位选择
          
          if (use_individual_heuristic == 1)
          {
              if (use_phase_saving)//相位保存策略
              {
                  int tid = qdpll->tail_saving[qdpll->num_tail_saving], did = decision_var->id;
                  if ((qdpll->_polarity[did] == 1 || qdpll->_polarity[did] == -1))
                  {
                      //printf("%d %d %d\n", did, tid, qdpll->skip_variable);
                      if (((tid != qdpll->skip_variable && tid != did) || neightbor_check(qdpll, walksat, qdpll->skip_variable, did) == 1))
                      //if (((tid != qdpll->skip_variable && tid != did) || neightbor_check(qdpll, walksat, qdpll->skip_variable, did) == 1) || 1)
                      //if (0)//变为0是全部使用相位保存策略，变为1是全部使用启发式策略
                      //if (((tid != qdpll->skip_variable) || neightbor_check(qdpll, walksat, qdpll->skip_variable, did) == 1))
                      {
                          //printf("select!!!!!!!  %d %d %d %d\n", qdpll->num_tail_saving, qdpll->tail_saving[qdpll->num_tail_saving], decision_var->id, qdpll->skip_variable);
                          //跳过刚刚，因为新的学习子句，而被强制传播赋值的变量
                          //如果刚刚翻转的变量和现在决策的did，同时出现在一个未被满足的子句里面，那么did的赋值有可能会被影响，
                          //即新选一遍，可能会和之前保存的相位不一样，因此在这里开始就改为选择策略，不再使用保存的相位
                          use_phase_saving = 0;
                          //if(qdpll->is_time_exit == 1)  use_phase_saving = 1;
                          assignment = select_decision_assignment(qdpll, decision_var);
                      }
                      else
                      {
                          
                          //if(decision_var->id == qdpll->skip_variable)  printf("saving----------------skip\n");
                          //else  printf("saving--------------------  %d %d %d\n", qdpll->num_tail_saving, qdpll->tail_saving[qdpll->num_tail_saving], decision_var->id);
                          assignment = qdpll->_polarity[did];
                      }
                      //printf("ass = %d\n", assignment);
                  }
                  else {
                      //printf("select(0)!!!!  %d %d %d %d\n", qdpll->num_tail_saving, qdpll->tail_saving[qdpll->num_tail_saving], decision_var->id, qdpll->skip_variable);
                      assignment = select_decision_assignment(qdpll, decision_var);
                  }
              }
              else  assignment = select_decision_assignment(qdpll, decision_var);
          }
          else {
              if (follow_ls_assignment)
              {
                  int _lit1 = decision_var->id;
                  //if (walksat->cur_soln[_lit1] == 0)  assignment = -1;
                  //else  assignment = 1;
                  assignment = (walksat->cur_soln[_lit1] << 1) - 1;//0->-1，1->1
              }
              else  assignment = select_decision_assignment(qdpll, decision_var);
          }

          //if(qdpll->only_phase_saving == 1)  assignment = qdpll->_polarity[decision_var->id];
          //assignment = select_decision_assignment(qdpll, decision_var);

          qdpll->tail_saving[qdpll->num_tail_saving++] = decision_var->id;
          qdpll->is_decision_variable[decision_var->id] = 1;
          
          //assignment = select_decision_assignment (qdpll, decision_var);
          /*
          if (qdpll->_polarity[decision_var->id] != 2)
          {
              assignment = qdpll->_polarity[decision_var->id];
          }
          else  assignment = select_decision_assignment(qdpll, decision_var);
          */
...
qdpll_delete (QDPLL * qdpll)
{
  ...
  qdpll_free(mm, qdpll->glue_decide_level, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  //qdpll_free(mm, qdpll->glue_decide_level, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll_free(mm, qdpll->variable_activity, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll_free(mm, qdpll->tend_ass_true, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  qdpll_free(mm, qdpll->tend_ass_false, (qdpll->pcnf.size_vars + 1) * sizeof(int));
  ...
  ccadical_release (qdpll->trivial_falsity_solver);
  ccadical_release (qdpll->trivial_truth_solver);
...
qdpll_configure (QDPLL * qdpll, char *configure_str)
{
  ...
  if (!strncmp (configure_str, "--trace", strlen ("--trace")))
  ...
  else if (!strncmp (configure_str, "--dep-man=", strlen ("--dep-man=")))
    {
      ...
      QDPLLDepManType _new;
      if (!strcmp (configure_str, "qdag"))
        _new = QDPLL_DEPMAN_TYPE_QDAG;
      else if (!strcmp (configure_str, "simple"))
        _new = QDPLL_DEPMAN_TYPE_SIMPLE;
      else
        {
      ...
      if (current != _new)
        {
          ...
          qdpll->dm =
            (QDPLLDepManGeneric *)
            qdpll_qdag_dep_man_create (qdpll->mm, &(qdpll->pcnf), _new,
                                       qdpll->options.
                                       depman_qdag_print_deps_by_search,
                                       qdpll);
          if (_new == QDPLL_DEPMAN_TYPE_QDAG)
            {
              assert (!qdpll->options.depman_qdag);
              qdpll->options.depman_qdag = 1;
            }
          else if (_new == QDPLL_DEPMAN_TYPE_SIMPLE)
            {
              assert (!qdpll->options.depman_simple);
              qdpll->options.depman_simple = 1;
            }
...
qdpll_get_relevant_clause_groups (QDPLL * qdpll)
{
  ...
  /* Do not use internal memory manager here because the user has to free the
     memory. */
  ClauseGroupID *relevant_clause_groups = (ClauseGroupID*) malloc (size);
                          ...
                          if (qdpll->state.qdo_trivial_truth_success)
                            {
                              ...
                              var->assignment = ccadical_val(qdpll->trivial_truth_solver, var->id);
...
qdpll_get_relevant_assumptions (QDPLL * qdpll)
{
  ...
  LitID *rel_assumptions = (LitID*) malloc (size);
