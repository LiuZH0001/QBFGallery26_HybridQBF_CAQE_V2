...
pq_enlarge (QDPLLMemMan * mm, EdgePriorityQueue * pq)
{
  ...
  Edge **_new = (Edge **) qdpll_malloc (mm, new_size * sizeof (Edge *));
  memcpy (_new, pq->elems_start, old_size * sizeof (Edge *));
  ...
  while (lits <= rightlit_p)
    {
      ...
      可能的优化：只需平衡一次，之后可将插入位置重用于同一作用域中的所有变量。*/
                                  ...
                                  const QDPLLDepManType type)
{
  ...
  VarID _class, next;
  for (_class = rep->GETQDAG (type).uf[offset].class_link.next; _class;
       _class = next)
    {
      Var *cur = VARID2VARPTR (vars, _class);
  ...
  for (s = prev ? prev->link.prev : 0; s; s = s->link.prev)
    {
      ...
      Var *class_rep;
      VarID _class;
      for (_class = s->GETPART (type).classes[1].first; _class;
           _class = class_rep->GETQDAG (type).uf[1].class_link.next)
        {
          class_rep = VARID2VARPTR (vars, _class);
...
static void
collect_class_as_candidate (QDPLLDepManQDAG * dm, Var * vars, Var * _class)
{
  assert (UF_IS_REP (_class, 0, type));
  assert (!_class->GETQDAG (type).mark_is_candidate);
  _class->GETQDAG (type).mark_is_candidate = 1;
  VAR_LINK (vars, dm->candidates, _class, GETQDAG (type).cand_link);
#if COMPUTE_STATS
  dm->stats.num_total_notinact_costs++;
  if (!_class->GETQDAG (type).mark_notified_inactive)
    {
      assert (dm->stats.num_cur_active_cands < dm->pcnf->used_vars);
      dm->stats.num_cur_active_cands++;
    }
#endif
  if (!UF_IS_SINGLETON_SET (_class, 0, type))
    {
      VarID mid;
      Var *m;
      for (mid = _class->GETQDAG (type).uf[0].members.list.first;
           mid; mid = m->GETQDAG (type).uf[0].members.link.next)
        {
          m = VARID2VARPTR (vars, mid);
          assert (!m->GETQDAG (type).mark_is_candidate);
...
static void
undo_collect_class_as_candidate (QDPLLDepManQDAG * dm, Var * vars, Var * _class)
{
  assert (UF_IS_REP (_class, 0, type));
  assert (_class->GETQDAG (type).mark_is_candidate);
  _class->GETQDAG (type).mark_is_candidate = 0;

  if (_class->GETQDAG (type).cand_link.prev
      || _class->GETQDAG (type).cand_link.next
      || _class->id == dm->candidates.first)
    VAR_UNLINK (vars, dm->candidates, _class, GETQDAG (type).cand_link);
...
qdpll_dep_man_init (QDPLLDepManGeneric * dmg)
{
  ...
  /* Traverse all scopes, make-set on all variables, link singleton
     ...
     scope by user. 
     遍历所有作用域，对所有变量执行 make-set 操作，将单例类链接至作用域的等价类列表。
注意：此操作也可在用户将变量添加到作用域之后立即执行。*/
  Scope *s;
  for (s = dm->pcnf->scopes.first; s; s = s->link.next)
    {
      VarID *p, *e;
...
qdpll_dep_man_get_candidates (QDPLLDepManGeneric * dmg)
{
  ...
  /* Allocate zero-terminated array of candidates. Caller is
     ...
     'free' afterwards. */
  LitID *result = (LitID*) malloc ((cnt + 1) * sizeof (LitID));
