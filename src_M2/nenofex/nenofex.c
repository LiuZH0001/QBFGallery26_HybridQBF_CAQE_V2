...
#define sat_solver_init(N) ((N)->ccadical = ccadical_init())
#define sat_solver_verbosity_mode(N) (ccadical_set_option((N)->ccadical, "verbose", 1))
#define sat_solver_reset(N) (ccadical_release((N)->ccadical))
#define sat_solver_add(N,lit) (ccadical_add((N)->ccadical,(lit)))
#define sat_solver_limit(N) (ccadical_limit((N)->ccadical, "decisions", (N)->options.sat_solver_dec_limit > 0 ? \
                                       (N)->options.sat_solver_dec_limit : -1))
#define sat_solver_sat(N) (ccadical_sat((N)->ccadical))
#define sat_solver_deref(N,lit) (ccadical_val((N)->ccadical,lit))

#define SAT_SOLVER_RESULT_UNKNOWN 0
#define SAT_SOLVER_RESULT_SATISFIABLE 10
#define SAT_SOLVER_RESULT_UNSATISFIABLE 20
...
compare_priority_queue (Stack * priority_heap, int pos_a, int pos_b)
{
  ...
  Var *var_a = (Var*) start[pos_a];
  Var *var_b = (Var*) start[pos_b];
...
delete_elem_priority_queue (Stack * priority_heap, int elem_pos)
{
  ...
  last = (Var*) pop_stack (priority_heap);
...
remove_min (Stack * priority_heap)
{
  ...
  last_var = (Var*) pop_stack (priority_heap);
...
init_variable (Nenofex * nenofex, unsigned int abs_lit, Scope * scope)
{
  ...
  if (!scope)                   /* add variable to default scope */
    {
      scope = (Scope*) nenofex->scopes->elems[0];
                                               ...
                                               Var * universal_var)
{
  ...
  while (count_stack (depending_existential_variables))
    {
      ...
      while ((depending_var =  (Var *) pop_stack (depending_existential_variables)))
        {
  ...
  while ((depending_var =(Var *) pop_stack (processed_existential_variables)))
    {
  ...
  Node *node;
  while ((node =(Node*) pop_stack (dependency_marked_nodes)))
    {
...
copy_formula (Nenofex * nenofex, Node * root)
{
  ...
  Node *cur;
  while ((cur =(Node*) pop_stack (node_stack)))
    {
      if (is_literal_node (cur))
        {
        ...
        {                       /* op-node -> copy children first */
          if (!copy_formula_marked (cur))
          ...
          else                  /* all children have been copied -> create OP-node */
            {
              ...
              Node *child =(Node*) pop_stack (copy_stack);
              ...
              for (i = 0; i < cnt; i++)
                {
                  child =(Node*) pop_stack (copy_stack);
  ...
  result =(Node*) pop_stack (copy_stack);
  assert (result);
...
post_expansion_flattening_check_for_cnf_form (Nenofex *nenofex, Node * split_or)
{
  ...
  Node *cur;
  while ((cur =(Node*) pop_stack (stack)) && result)
    {
      if (!is_literal_node (cur))
        {
          Node *child;
...
expand_universal_variable (Nenofex * nenofex, Var * var)
{
  ...
  if (count_stack (nenofex->depending_vars))
    {                           /* expansion of non-innermost universal variable -> need to clean up dep. vars */
      ...
      Var *depending_var;
      while ((depending_var =(Var*) pop_stack (nenofex->depending_vars)))
        {
          assert (is_existential_scope (depending_var->scope));
          assert (depending_var->scope->nesting >
...
AGAIN:
  ...
  Node *collected_node;
  while ((collected_node = (Node*)pop_stack (collected_nodes)))
    {
...
{
  Node *_and = (Node*)and_node (nenofex);
  _and->size_subformula = 1;     /* will be incremented when adding clauses */
  nenofex->graph_root = _and;
}
                                        ...
                                        Stack * node_stack)
{
  ...
  Node *node;
  while ((node = (Node*)pop_stack (node_stack)))
    {
...
simplify_eliminate_unates (Nenofex * nenofex)
{
  ...
  Var *var;
  while ((var =(Var *) pop_stack (nenofex->unates)))
    {
...
collected_variables_update_scores (Nenofex * nenofex)
{
  ...
  Var *update_var;
  while ((update_var =(Var *) pop_stack (collected_variables)))
    {
...
AGAIN:
  ...
  Var *var;
  while ((var =(Var *) pop_stack (cur_scope_vars)))
...
print_lit_stats_before_exp (Nenofex * nenofex, Var * var)
{
  ...
  while ((childp = (Node*)pop_stack (stack)))
    {
      ...
      if (forward)
        {
          ...
          nenofex->result =(NenofexResult) sat_res;
...
nenofex_create ()
{
  ...
  Nenofex *result =(Nenofex *) mem_malloc (mm, num_bytes);
  assert (result);
  memset (result, 0, num_bytes);
...
nenofex_delete (Nenofex *nenofex)
{
  ...
  Scope *scope;
  while ((scope =(Scope *) pop_stack (nenofex->scopes)))
    {
      Var *var;
      while ((var =(Var *) pop_stack (scope->vars)))
        {
...
nenofex_parse (Nenofex * nenofex, FILE * input_file)
{
  ...
  ScopeType parsed_scope_type = (ScopeType)0;
