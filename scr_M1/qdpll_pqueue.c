...
{
  PriorityQueue *pq = (PriorityQueue*) qdpll_malloc (mm, sizeof (PriorityQueue));
  if (init_size == 0)
    init_size = 1;
  PriorityQueueElem *queue = (PriorityQueueElem*) qdpll_malloc (mm, sizeof (PriorityQueueElem) * init_size);
  pq->queue = queue;
  pq->size = init_size;
  assert (pq->cnt == 0);
  PriorityQueueElem *p, *e;
...
pqueue_adjust (QDPLLMemMan * mm, PriorityQueue * pqueue, unsigned int size)
{
  ...
  if ((old_size = pqueue->size) < size)
    {
      pqueue->queue = (decltype(pqueue->queue)) qdpll_realloc (mm, pqueue->queue,
